...
export function createStatsReader(d) {
  const {
    ...
    checkTokenQuota, listProfiles, listQuotaPools, getProfileInflight,
  } = d;
...
// only(可选):只跑这些键的查询。dashboard 按菜单懒加载时用 —— 点「错误记录」就不必跑
// dailyModels / dailyClients / hourlyModels 这三张 400 天宽表的全表 GROUP BY。
// 不传 = 全跑(与历史行为完全一致,测试依赖这一点)。
function getAggregatedStore(suffixFilter, { only = null } = {}) {
  const want = (k) => !only || only.has(k);
  const agg = { users: {}, daily: {}, dailyModels: {}, dailyClients: {}, dailyHourly: {}, models: {}, hourly: {}, errors: [] };
  ...
  if (want("dailyClients")) {
    // dailyClients: GROUP BY date, user_key, client。与 dailyModels 同形(同样不带 cache 列),
  }

  // 所以前端那份按日期范围 + USR 筛选的逻辑可以两处共用。客户端与协议正交 —— 这一轴不受
  // suffixFilter 之外的任何协议筛选影响,和模型维度一样是横跨所有方案的。
  // client='unknown' 是真实的行(不是缺失),表内合计恒等于总量,没有需要披露的缺口。
  for (const r of q(`SELECT date, user_key, client, SUM(input_tokens) AS ti, SUM(output_tokens) AS tout, SUM(requests) AS tr FROM usage_daily_client __WHERE__ GROUP BY date, user_key, client`)) {
    if (!agg.dailyClients[r.date]) agg.dailyClients[r.date] = {};
    if (!agg.dailyClients[r.date][r.user_key]) agg.dailyClients[r.date][r.user_key] = {};
    agg.dailyClients[r.date][r.user_key][r.client] = { inputTokens: r.ti||0, outputTokens: r.tout||0, requests: r.tr||0 };
  }
  if (want("dailyHourly")) {
...
// 方案中心「使用状态」的活跃窗口:该方案有进行中的请求(inflight),或最近 5 分钟内
// 有过**完成记账**的请求(users.last_active),即「使用中」。
// 单次编码请求几秒就结束而面板 30 秒才轮询,严格「此刻进行中」几乎永远显示休眠,
// 所以窗口是必要的;5 分钟是产品约定,不进配置 —— 改这里 + 改 wiki 即可。
const ACTIVE_WINDOW_MS = 5 * 60 * 1000;

function getProfileSummaries() {
  const today = cnDate();
  // 一次性取数,避免每个方案各发一条 SQL;inflight 是普通对象快照,读期间不会被改
  const inflight = getProfileInflight();
  const lastActiveBySuffix = {};
  for (const r of stmts.profileLastActive.all()) lastActiveBySuffix[r.profile] = r.la;
  return listProfiles().map(profile => {
    ...
    const la = lastActiveBySuffix[profile.suffix] || null;
    const laMs = la ? new Date(la).getTime() : NaN;   // 坏串容错:NaN 走休眠分支
    const infl = inflight[profile.suffix] || 0;
    return {
      name: profile.name,
      ...
      // 「使用状态」:active 在后端算好是单一事实来源,前端只管渲染。
      // inflight>0 兜底了「长流式请求 last_active 已超窗」的场景,两条件互补无死角。
      active: infl > 0 || (Number.isFinite(laMs) && Date.now() - laMs < ACTIVE_WINDOW_MS),
      inflight: infl,
      lastActive: la,
