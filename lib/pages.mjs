...
<div class="note" id="mnNote" style="font-size:11px;margin-bottom:10px">渠道凭据只存在服务器上,这里只显示是否已配置;输入框留空表示不修改。</div>
<label style="display:flex;align-items:center;gap:6px;margin-bottom:12px;cursor:pointer"><input type="checkbox" id="mnEnabled" style="width:auto;accent-color:var(--accent)"><span style="font-size:12.5px">启用我的评审通知</span></label>
<div class="row" style="grid-template-columns:1fr 1fr;gap:10px">
  <div><label>飞书机器人 Webhook</label><input type="text" id="mnFeishu" placeholder="https://open.feishu.cn/open-apis/bot/v2/hook/..."></div>
  <div><label>钉钉机器人 Webhook</label><input type="text" id="mnDingtalk" placeholder="https://oapi.dingtalk.com/robot/send?access_token=..."></div>
  <div><label>企业微信机器人 Webhook</label><input type="text" id="mnWecom" placeholder="https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=..."></div>
  <div><label>Server酱 SendKey</label><input type="text" id="mnServerchan" placeholder="SCT..."></div>
  <div><label>Bark Device Key</label><input type="text" id="mnBarkKey" placeholder="iOS 装 Bark 后复制的 Key"></div>
  <div><label>收件邮箱 <span class="lbl-hint">(需管理员配好 SMTP)</span></label><input type="text" id="mnEmail" placeholder="you@corp.com"></div>
</div>
<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
  <button type="button" class="btn btn-outline btn-sm" onclick="testMyNotify()">发送测试</button>
  <button type="button" class="btn btn-primary btn-sm" onclick="saveMyNotify()">保存</button>
</div>
<div class="inline-status" id="mnStatus" role="status"></div>
</div>
</div>
