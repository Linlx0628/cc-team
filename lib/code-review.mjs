...
export function createCodeReview(d) {
  ...
  // 待处理计数(菜单红点):自己名下仓库里「有意见、且晚于上次查看」的运行数
  function pendingFindings(apiKey, { lastSeenAt = null, repoIds = null } = {}) {
    const ids = repoIds || reposVisibleTo(apiKey).map((r) => r.id);
    if (!ids.length) return 0;
    const ph = ids.map(() => "?").join(",");
    const args = [...ids];
    let where = `repo_id IN (${ph}) AND comments_count > 0`;
    if (lastSeenAt) { where += " AND created_at > ?"; args.push(lastSeenAt); }
    return db().prepare(`SELECT COUNT(*) c FROM code_review_runs WHERE ${where}`).get(...args).c || 0;
  }
  ...
  function memberView(apiKey, { limit = 20, lastSeenAt = null } = {}) {
    ...
    const all = rows.filter((r) => allowed.has(r.repo_id));
    const mine = all.slice(0, Math.min(50, Math.max(1, limit)));
    // 待处理 = 「评出过意见、且在我上次打开之后」的运行数。用时间戳而非逐条状态:
    // 意见表没有已读列,而「上次看到哪里」这个语义用时间戳表达最省且够用。
    const pending = pendingFindings(apiKey, { lastSeenAt, repoIds: repos.map((r) => r.id) });
    return {
      enabled: !!cfg().enabled,
      pending,
      repos: repos.map((r) => {
  ...
  return {
    ...
    memberView, memberRun, listFindings, pendingFindings,
    // 仓库运维
