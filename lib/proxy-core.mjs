...
import { translateCompactionInputItems } from "./compact-bridge.mjs";
import { tryThinkingPassbackSelfHeal } from "./thinking-passback.mjs";
import { injectEmptyInputPlaceholder, tryEmptyInputSelfHeal } from "./empty-input-compat.mjs";
...
function proxyRequest(req, res) {
  ...
  const reqLog = {
    ...
    model: "",
    profile: "",
    // 客户端维度。client 是解析出的标识(展示用),ua 是原文 —— 留着是为了日后排查
    // 没见过的客户端:只有 UA 原文才看得出该往 CLIENT_LABELS 里补什么。
    // 日志只保 30 天且不入库,原文 UA 落在这里可接受。
    client: extractClientSignal(req.headers),
  ...
  // 调度组的成员同样在守卫范围内：判定集合是本协议**基础组与全部命名组的并集**。
  // 否则一个只出现在调度组里的方案可以带后缀直连，绕过「不许绕开 failover 直连贵方案」。
  // 并集是严格超集 —— 严格了，不放宽任何既有情形。启用条件（任一组 ≥2 人）因此也是
  // 有意的行为扩大：多成员组存在就意味着有 failover 可绕。逃生门是 restrictGroupSuffix=false。
  ...
  const groupNames = new Set(dpg);
  let anyGroupHasFailover = dpg.length >= 2;
  // 分桶存储：只扫**本协议**的桶。扁平表时代这里是全表扫描，改成分桶后若还遍历
  // 顶层会拿到两个桶对象（没有 members），全部被跳过 → 直连限制静默失效。
  for (const g of Object.values((config.scheduleGroups || {})[protocol] || {})) {
    if (!g || typeof g !== "object" || g.protocol !== protocol || !Array.isArray(g.members)) continue;
    for (const m of g.members) {
      if (typeof m === "string" && m) groupNames.add(m);
    }
    if (g.members.length >= 2) anyGroupHasFailover = true;
  }
  const groupEntryPath = protocol === "responses" ? "/v1/responses" : "/v1/messages";
  if (config.restrictGroupSuffix !== false && anyGroupHasFailover && !isSuperUser(apiKey, runtime) && !resolvedProfile.isDefaultEntry && groupNames.has(runtime.profileName)) {
    if (protocol === "responses") {
  ...
  readBody(req, 50_000_000).then(async (body) => {
    ...
    // 客户端标识。与协议无关,也不参与路由 —— 只透传给 recordUsage 记进客户端维度的表。
    // 这里算一次而不是在 recordUsage 里算:recordUsage 只看得到 apiKey/usage,拿不到 headers。
    const clientSignal = extractClientSignal(req.headers);
    ...
    // 生效组（方案组调度的结果）在本请求内只解析一次，并同时喂给候选生成、粘性判定与
    // failover 审计 —— 三处必须看到同一个组头，详见 server.mjs resolveRequestGroup。
    const effectiveGroup = resolvedProfile.isDefaultEntry
      ? resolveRequestGroup(protocol)
      : null;
    const effectiveHead = effectiveGroup ? (effectiveGroup.members[0] || null) : null;
    let candidateList = resolvedProfile.isDefaultEntry
      ? (protocol === "responses"
        ? getAvailableResponsesProfiles(apiKey, effectiveGroup.members)
        : getAvailableDefaultProfiles(apiKey, effectiveGroup.members))
    ...
    if (resolvedProfile.isDefaultEntry) {
      ...
      reqLog.group = effectiveGroup.groupName;
      reqLog.groupSource = effectiveGroup.source;
    }
    ...
    // 注意：这里回退到的是**基础组头**（resolveProfile 的默认入口初值），它可能根本不在生效组里。
    // 「规则组被换成基础组」是继承既有语义（宁可服务也不 429），本功能不改这条语义。
    // 留痕走 noteFailoverServed：审计里是一条 failover.switch，说清「生效组头不可用、由谁接手」。
    // 这里**不是** schedule.group_switch —— 生效组本身没变，变的只是这一次的实际服务方案，
    // 所以 lastServedGroup 的基线不该被这次兜底改写。
    ...
    try {
      for (let ci = 0; ci < candidateList.length; ci++) {
        ...
        // 顺带做一次 compaction 回放翻译:客户端把上一轮压缩得到的 compaction 条目
        // 存进历史,后续轮次原样回放(见 lib/compact-bridge.mjs)——上游不认识该类型,
        // 换成带摘要原文的 message。两种改写合并成一次序列化。
        ...
        try {
          ...
          let dirty = false;
          if (resolved !== reqModel) { parsed.model = resolved; creqModel = resolved; dirty = true; }
          if (cruntime.protocol === "responses") {
            if (translateCompactionInputItems(parsed)) dirty = true;
            // sticky 已置位(该上游拒绝过空 input)→ 直接带占位发,不再吃一次 400 往返
            if (cruntime.requiresInputItems && injectEmptyInputPlaceholder(parsed)) dirty = true;
          }
          if (dirty) cbody = Buffer.from(JSON.stringify(parsed));
        } catch {}
        ...
        try {
          ...
          // 「使用状态」计数同点位 acquire: 该候选开始向上游派发时 +1,
          // finally 里必然配对 -1 —— 覆盖成功/失败/取消/failover 的所有出口。
          acquireProfileInflight(csuffix);
          reqLog.profile = cand.name;
          if (isStreamRequest) {
            await handleStreamingProxy(req, res, cbody, reqHeaders, apiKey, creqModel, timeout, reqSource, cruntime, csuffix, candStrippedUrl, clientState, sessionSignal, clientSignal);
          } else {
            await handleJsonProxy(req, res, cbody, reqHeaders, apiKey, creqModel, timeout, reqSource, cruntime, csuffix, candStrippedUrl, clientState, sessionSignal, clientSignal);
          }
          served = true;
          servedBy = cand.name;
          break;
          ...
          // 与派发前的 acquireProfileInflight 配对;continue/break/异常路径都会经过这里
          releaseProfileInflight(csuffix);
        }
      }
      ...
      // **当前生效**组头（方案组调度生效时它不是基础组头）。A normal turn served by the
      // head refreshes it (cache affinity across turns); a turn served by a failover
      // member clears it — so once the head recovers from a limit/breaker it naturally
      // returns to the front instead of the conversation staying pinned to the fallback.
      //
      // 这里必须用生效组头而不是基础组头：规则生效期间由规则组头（如 GLM）服务时，
      // 拿基础组头（如 DeepSeek）比较会得出 servedBy !== headName，于是**每一轮都清掉绑定**
      // —— 跨轮 prompt 缓存亲和静默失效，不报错、只是变慢变贵。
      if (servedBy && resolvedProfile.isDefaultEntry && sessionSignal) {
        const headName = effectiveHead;
      ...
      // 组头取生效组头，理由同上：否则计划内的调度切换会被写进审计，谎称组头
      // 「被限流/熔断/不可用」。真实的调度切换由 noteGroupSwitch 单独写 schedule.group_switch。
      if (servedBy && resolvedProfile.isDefaultEntry) {
        const userName = getUserName(apiKey, runtime);
        noteGroupSwitch(protocol, effectiveGroup, userName);
        noteFailoverServed(protocol, servedBy, userName, effectiveHead);
      }
    } finally {
...
// session 是 extractSessionSignal 的产出,由 proxyRequest 的闭包透传进来(模块级函数
// 看不到那个 const)。只用于把 token 记进 usage_session,不参与路由决策。
// client 同理(extractClientSignal),只用于客户端维度的记账。
async function handleJsonProxy(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session, client) {
  ...
  for (let attempt = 0; attempt <= gProxy.maxRetries + (healUsed ? 1 : 0); attempt++) {
    try {
      ...
      // handing the user a 400 they cannot act on. 同一位置并列处理「thinking 必须
      // 回传」的 400(第三方 Anthropic 兼容端点 + CC 丢 thinking 块/中途换厂商,
      // 见 lib/thinking-passback.mjs):同样只重发一次,两种修复共用 healUsed。
      if (!healUsed) {
        ...
        const thinkingHealed = tryThinkingPassbackSelfHeal(body, upRes.statusCode, text);
        if (thinkingHealed) {
          console.log(`[自愈] thinking 回传修复后重试 profile=${suffix} model=${reqModel}`);
          reqHeaders["content-length"] = thinkingHealed.length;   // 体变了,长度必须跟着变(与 toolPattern 自愈同)
          body = thinkingHealed; healUsed = true; continue;
        }
        // 空 input 被拒(DeepSeek 等无状态端点):注入占位重发并置 sticky,见 lib/empty-input-compat.mjs
        const emptyInputHealed = tryEmptyInputSelfHeal(body, upRes.statusCode, text, runtime);
        if (emptyInputHealed) {
          reqHeaders["content-length"] = emptyInputHealed.length;
          body = emptyInputHealed; healUsed = true; continue;
        }
      }
      ...
      try {
        ...
        if (upRes.statusCode >= 400) {
          ...
          if (usage) {
            recordUsage(apiKey, usage, json.model, suffix, runtime, session, client);
...
async function streamUpstreamOnce(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session, client) {
  const runtime = _rt || d.rt;
  throwIfClientAborted(clientState);
  const opts = {
    ...
    const upReq = transport.request(opts, (upRes) => {
      ...
      if (upRes.statusCode >= 400) {
        ...
        upRes.on("end", () => {
          ...
          // 同一条 400 链路并列处理「thinking 必须回传」(见 lib/thinking-passback.mjs)
          const thinkingHealed = !clientGone && tryThinkingPassbackSelfHeal(body, upRes.statusCode, errBuf);
          if (thinkingHealed) {
            console.log(`[自愈] thinking 回传修复后重试 profile=${suffix} model=${reqModel}`);
            reqHeaders["content-length"] = thinkingHealed.length;   // 体变了,长度必须跟着变
            const retry = new Error("thinking-passback resend");
            retry.thinkingRetry = true;
            retry.body = thinkingHealed;
            safeReject(retry);
            upReq.destroy();
            return;
          }
          // 以及「空 input 被拒」(DeepSeek 等):注入占位重发并置 sticky
          const emptyInputHealed = !clientGone && tryEmptyInputSelfHeal(body, upRes.statusCode, errBuf, runtime);
          if (emptyInputHealed) {
            reqHeaders["content-length"] = emptyInputHealed.length;
            const retry = new Error("empty-input resend");
            retry.emptyInputRetry = true;
            retry.body = emptyInputHealed;
            safeReject(retry);
            upReq.destroy();
            return;
          }
      ...
      upRes.on("end", () => {
        ...
        if (usageHasTokens(usage)) {
          recordUsage(apiKey, usage, model, suffix, runtime, session, client);
...
async function handleStreamingProxy(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session, client) {
  let healUsed = false;
  for (;;) {
    try {
      await streamUpstreamOnce(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session, client);
      return;
    } catch (err) {
      // 三种自愈(tool-pattern / thinking 回传 / 空 input)共用一次重发机会,与 handleJsonProxy 同语义
      if ((!err?.toolPatternRetry && !err?.thinkingRetry && !err?.emptyInputRetry) || healUsed) throw err;
      healUsed = true;
      body = err.body;
    }
  }
