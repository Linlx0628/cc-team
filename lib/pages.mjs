...
export function settingsHtml(d, errorMsg) {
  ...
  // 方案组调度正在接管某协议（active 指向命名组）时，侧栏方案组编辑器会被只读提示替换：
  // dock 里的编辑器编辑的是基础组，此时展示它会让管理员误以为路由的就是这套顺序。
  // 规则指向 @base 时生效组就是基础组，仍保留编辑器。hidden 只在这里与客户端
  // applyScheduleDockState()（settings.js）里翻转 —— 切换靠 [hidden]，不能给
  // .sidebar-global 加任何 display: CSS 规则，否则作者 CSS 会压过 [hidden] 让切换失效。
  const schedProtos = (s.schedule && s.schedule.protocols) || {};
  const schedSteers = (proto) => {
    const a = schedProtos[proto] && schedProtos[proto].active;
    return !!(a && a.group && a.group !== "@base");
  };
  // 标题固定短语，组名与来源放下面的换行说明里 —— 侧栏窄，标题里塞规则摘要必然截断。
  const schedHintGroup = (proto) => {
    const a = (schedProtos[proto] || {}).active || {};
    return escHtml(a.group || "");
  };
  const schedHintSrc = (proto) => {
    const a = (schedProtos[proto] || {}).active || {};
    return escHtml(a.source === "manual" ? "手动指定" : (a.ruleSummary || "调度规则"));
  };
...
<div class="pl-actions">
  ...
  ${'<button class="pl-activate" onclick="event.stopPropagation();copyProfileCode(this,\'' + escJs(p.name) + '\')">复制代码</button>'}
...
<div class="sidebar-dock">
  <div class="sidebar-global" data-proto="anthropic" style="padding:8px 12px"${schedSteers("anthropic") ? " hidden" : ""}>
    <div style="font-size:11px;font-weight:650;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between">
      ...
      <div style="display:flex;align-items:center;gap:6px;flex:none">
        ...
        <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;font-weight:400;white-space:nowrap" title="全局设置：同时作用于两个协议的方案组；勾选后**基础组与调度组的成员**都不允许带 /方案后缀直连（直连会绕过 failover，等于绕开限额降级）"><input type="checkbox" id="restrictGroupSuffixCb" class="restrict-group-suffix-cb" ${config.restrictGroupSuffix !== false ? "checked" : ""} onchange="setRestrictGroupSuffix(this.checked)" style="width:auto;accent-color:var(--accent)"> 限制直连</label>
      </div>
    </div>
  ...
  <div class="sidebar-global" data-proto="anthropic" id="schedDockHint-anthropic" style="padding:8px 12px"${schedSteers("anthropic") ? "" : " hidden"}>
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px">
      <span style="font-size:11px;font-weight:650;white-space:nowrap">方案组调度生效中</span>
      <button type="button" class="btn btn-outline btn-sm" onclick="openPlanScheduleView()" style="font-size:10px;padding:3px 8px;flex:none">查看调度</button>
    </div>
    <div style="font-size:11px;color:var(--dim);margin-bottom:6px">当前按「<span id="schedDockHintText-anthropic">${schedHintGroup("anthropic")}</span>」路由（<span id="schedDockHintSrc-anthropic">${schedHintSrc("anthropic")}</span>）。侧栏方案组暂不生效，规则结束后自动恢复编辑。</div>
    <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px" title="全局设置：同时作用于两个协议的方案组；勾选后**基础组与调度组的成员**都不允许带 /方案后缀直连（直连会绕过 failover，等于绕开限额降级）"><input type="checkbox" class="restrict-group-suffix-cb" ${config.restrictGroupSuffix !== false ? "checked" : ""} onchange="setRestrictGroupSuffix(this.checked)" style="width:auto;accent-color:var(--accent)"> 限制直连</label>
  </div>
  <div class="sidebar-global" data-proto="responses" style="padding:8px 12px;display:none"${schedSteers("responses") ? " hidden" : ""}>
    <div style="font-size:11px;font-weight:650;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between">
      ...
      <div style="display:flex;align-items:center;gap:6px;flex:none">
        ...
        <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;font-weight:400;white-space:nowrap" title="全局设置：同时作用于两个协议的方案组；勾选后**基础组与调度组的成员**都不允许带 /方案后缀直连（直连会绕过 failover，等于绕开限额降级）"><input type="checkbox" id="restrictGroupSuffixCb2" class="restrict-group-suffix-cb" ${config.restrictGroupSuffix !== false ? "checked" : ""} onchange="setRestrictGroupSuffix(this.checked)" style="width:auto;accent-color:var(--accent)"> 限制直连</label>
      </div>
    </div>
  ...
  <div class="sidebar-global" data-proto="responses" id="schedDockHint-responses" style="padding:8px 12px;display:none"${schedSteers("responses") ? "" : " hidden"}>
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px">
      <span style="font-size:11px;font-weight:650;white-space:nowrap">方案组调度生效中</span>
      <button type="button" class="btn btn-outline btn-sm" onclick="openPlanScheduleView()" style="font-size:10px;padding:3px 8px;flex:none">查看调度</button>
    </div>
    <div style="font-size:11px;color:var(--dim);margin-bottom:6px">当前按「<span id="schedDockHintText-responses">${schedHintGroup("responses")}</span>」路由（<span id="schedDockHintSrc-responses">${schedHintSrc("responses")}</span>）。侧栏方案组暂不生效，规则结束后自动恢复编辑。</div>
    <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px" title="全局设置：同时作用于两个协议的方案组；勾选后**基础组与调度组的成员**都不允许带 /方案后缀直连（直连会绕过 failover，等于绕开限额降级）"><input type="checkbox" class="restrict-group-suffix-cb" ${config.restrictGroupSuffix !== false ? "checked" : ""} onchange="setRestrictGroupSuffix(this.checked)" style="width:auto;accent-color:var(--accent)"> 限制直连</label>
  </div>
<div class="sidebar-nav">
  ...
  <button type="button" class="nav-btn" id="planScheduleNav" onclick="openPlanScheduleView()" title="方案组调度——预先排列多套方案组，按星期与时段自动切换优先级">方案组调度</button>
  <button type="button" class="nav-btn" id="codeReviewNav" onclick="openCodeReviewView()" title="代码评审——按仓库白名单拉代码做行级评审">代码评审</button>
</div>
<div class="sidebar-ft" style="display:flex;gap:6px"><button class="btn btn-outline btn-sm" id="userManageNav" onclick="openUserManageView()" style="flex:1">用户管理</button><button class="btn btn-outline btn-sm" id="notifyManageNav" onclick="openNotifyManageView()" style="flex:1">通知管理</button></div>
</div>
</div>
<div class="main">
${errDiv}
...
<div class="section">
  ...
  <div class="note" style="margin:12px 0 8px">邮件（SMTP 直发）—— 用于把代码评审发现的问题发给提交人。465 是隐式 TLS（默认），587 请在下面把「隐式 TLS」取消勾选（走 STARTTLS）。密码框留空 = 不修改已保存的密码。</div>
  <div class="row" style="grid-template-columns:1fr 1fr;gap:10px">
    <div><label>SMTP 服务器</label><input type="text" id="notifSmtpHost" placeholder="smtp.exmail.qq.com" style="font-family:var(--font-mono);font-size:11px"></div>
    <div><label>发件人邮箱</label><input type="text" id="notifSmtpFrom" placeholder="review@your-domain.com" style="font-family:var(--font-mono);font-size:11px"></div>
    <div><label>端口</label><input type="number" id="notifSmtpPort" min="1" max="65535" placeholder="465" style="font-family:var(--font-mono);font-size:11px"></div>
    <div><label>账号 <span class="lbl-hint">(SMTP 认证用户名,填**完整邮箱地址**;留空 = 匿名发信)</span></label><input type="text" id="notifSmtpUser" placeholder="review@your-domain.com" style="font-family:var(--font-mono);font-size:11px"></div>
    <div><label>密码 / 授权码 <span class="lbl-hint">(明文显示,方便核对;清空保存 = 保留原值)</span></label><input type="text" id="notifSmtpPass" autocomplete="off" style="font-family:var(--font-mono);font-size:11px"></div>
    <div><label>默认收件人（可选）</label><input type="text" id="notifSmtpTo" placeholder="留空则测试邮件发给自己" style="font-family:var(--font-mono);font-size:11px"></div>
    <div><label>发件人显示名 <span class="lbl-hint">(可选,收件人看到的「发件人」名字)</span></label><input type="text" id="notifSmtpFromName" placeholder="如: CC-TEAM-代码评审"></div>
    <div></div>
  </div>
  <div style="display:flex;gap:18px;flex-wrap:wrap;margin-top:8px">
    <label style="display:flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" id="notifSmtpSecure" style="width:auto;accent-color:var(--accent)"><span style="font-size:12.5px">使用隐式 TLS（465 勾选；587 取消勾选走 STARTTLS）</span></label>
    <label style="display:flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" id="notifSmtpInsecure" style="width:auto;accent-color:var(--accent)"><span style="font-size:12.5px">跳过证书校验（仅内网自签名证书时勾选）</span></label>
  </div>
...
${(() => {
  ...
  return pools.map(p => {
    ...
    const memberChips = p.profiles.map(m => `<span class="tag" style="background:rgba(0,0,0,.04);color:${m.protocol === "responses" ? "var(--blue)" : "var(--accent)"}">${escHtml(m.name)}${m.protocol === "responses" ? " · OpenAI" : " · Anthropic"}</span>`).join(" ");
...
<div id="codeReviewView" hidden aria-hidden="true">
<h2>代码评审 <span class="status" id="crStatusPill">未启用</span></h2>
<div class="note" style="margin-bottom:12px">系统按下方仓库白名单把代码拉到本地,用 <a href="https://open-codereview.ai" target="_blank" rel="noopener">open-code-review</a> 做行级评审。评审经<strong>本网关</strong>调用你选的方案,消耗计入对应账号的用量与配额;需要宿主机已安装 <code>git ≥ 2.41</code> 与 <code>npm i -g @alibaba-group/open-code-review</code>(网关不会自动安装)。功能会在服务器上执行 git 与 ocr 命令,默认关闭。</div>

<div class="section">
  <label class="checkline"><input type="checkbox" id="crEnabled" class="cbx"><span style="font-weight:600">启用代码评审</span></label>
  <div class="row" style="margin-top:12px">
    <div>
      <label>评审用哪个方案 <span class="lbl-hint">(评审请求经本网关发往该方案)</span></label>
      <select id="crProfile" onchange="crOnProfileChange()"></select>
    </div>
    <div>
      <label>评审模型 <span class="lbl-hint">(该方案可用的别名)</span></label>
      <select id="crModel"></select>
    </div>
  </div>
  <div class="row" style="margin-top:12px">
    <div>
      <label>评审账号 <span class="lbl-hint">(用量记在该账号名下,便于对账)</span></label>
      <input type="text" id="crKeyText" readonly placeholder="尚未创建">
    </div>
    <div style="display:flex;align-items:flex-end;gap:8px">
      <button type="button" class="btn btn-outline btn-sm" onclick="createReviewAccount()">创建评审账号</button>
      <button type="button" class="btn btn-outline btn-sm" onclick="probeReviewEngine()">重新检测引擎</button>
      <button type="button" class="btn btn-outline btn-sm" onclick="runReviewSelfCheck()">运行自检</button>
    </div>
  </div>
  <div class="note" id="crEngineNote" style="margin-top:10px"></div>
  <div id="crSelfCheck" hidden style="margin-top:10px"></div>
  <div class="inline-status" id="crStatus" role="status"></div>
</div>

<div class="section">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
    <strong style="font-size:13px">仓库白名单</strong>
    <span class="note" style="margin:0">只有这里登记的仓库会被拉取;成员无法提交任意地址</span>
    <span style="flex:1"></span>
    <button type="button" class="btn btn-outline btn-sm" onclick="addReviewRepo()">＋ 添加仓库</button>
  </div>
  <div style="overflow-x:auto">
    <table><thead><tr><th>名称</th><th>来源</th><th>地址 / 路径</th><th>分支</th><th>凭据</th><th>定时</th><th>在线触发</th><th>成员</th><th>操作</th></tr></thead>
    <tbody id="crRepoBody"></tbody></table>
  </div>
  <div id="crRepoEditor" hidden style="margin-top:12px;padding-top:12px;border-top:1px dashed var(--border)">
    <div class="row">
      <div><label>名称</label><input type="text" id="crRepoName" placeholder="例如 后端主仓"></div>
      <div><label>来源</label><select id="crRepoSource" onchange="crOnRepoSourceChange()"><option value="remote">remote(远程仓库)</option><option value="local">local(服务器本地目录)</option></select></div>
    </div>
    <div class="row" style="margin-top:10px">
      <div><label id="crRepoUrlLabel">仓库地址</label><input type="text" id="crRepoUrl" placeholder="https://github.com/org/repo.git"></div>
      <div><label>分支 <span class="lbl-hint">(可直接输入;点「拉取分支」或 ▾ 从仓库分支里选)</span></label>
        <div style="display:flex;gap:6px">
          <div style="flex:1;position:relative">
            <input type="text" id="crRepoBranch" placeholder="main" autocomplete="off" role="combobox" aria-expanded="false" style="width:100%;padding-right:26px">
            <button type="button" id="crRepoBranchArrow" title="展开分支列表" style="position:absolute;right:2px;top:50%;transform:translateY(-50%);border:0;background:transparent;cursor:pointer;color:var(--dim);font-size:9px;padding:2px 6px">▼</button>
            <div id="crRepoBranchDrop" class="cr-branch-drop" hidden></div>
          </div>
          <button type="button" class="btn btn-outline btn-sm" onclick="fetchReviewRepoBranches()">拉取分支</button>
        </div>
      </div>
    </div>
    <div class="row" style="margin-top:10px">
      <div><label>凭据方式</label><select id="crRepoAuth"><option value="none">无凭据(公开仓库 / 本地目录)</option><option value="token">HTTPS Token</option><option value="sshKey">SSH 私钥路径</option></select></div>
      <div><label>凭据 <span class="lbl-hint">(明文显示,可直接修改;清空保存 = 保留原值)</span></label><input type="text" id="crRepoCred" placeholder="Token 或私钥绝对路径"></div>
    </div>
    <div class="row3" style="margin-top:10px">
      <div><label>定时评审</label><select id="crRepoSched"><option value="off">关闭</option><option value="interval">固定间隔</option><option value="daily">每天定时</option></select></div>
      <div><label>间隔(小时)</label><input type="number" id="crRepoInterval" min="1" max="168" value="6"></div>
      <div><label>每天时刻(北京时间)</label><input type="text" id="crRepoAt" placeholder="03:00"></div>
    </div>
    <div class="row" style="margin-top:10px">
      <div><label>只在星期几 <span class="lbl-hint">(1=周一…7=周日,逗号分隔;留空=每天;仅「每天定时」用)</span></label><input type="text" id="crRepoWeekdays" placeholder="1,2,3,4,5"></div>
    </div>
    <label class="checkline"><input type="checkbox" id="crRepoApiTrigger" class="cbx"><span>允许通过 API / MCP 触发该仓库</span></label>
    <label class="checkline"><input type="checkbox" id="crRepoPushTrigger" class="cbx"><span>push 到该仓库时自动评审(Webhook)</span></label>
    <div style="margin-top:12px">
      <label>负责这个仓库的成员 <span class="lbl-hint">(勾中的人 = 能触发它 + 能在「我的用量」页看到它的评审结果)</span></label>
      <div style="overflow-x:auto;margin-top:6px;max-height:220px;overflow-y:auto">
        <table><thead><tr><th style="width:40px">加入</th><th>成员</th><th>虚拟 Key</th></tr></thead>
        <tbody id="crRepoMemberBody"></tbody></table>
      </div>
    </div>
    <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px">
      <button type="button" class="btn btn-outline btn-sm" onclick="cancelReviewRepoEdit()">取消</button>
      <button type="button" class="btn btn-outline btn-sm" onclick="testReviewRepoForm()">测试连接</button>
      <button type="button" class="btn btn-primary btn-sm" onclick="saveReviewRepo()">保存仓库</button>
    </div>
    <div class="inline-status" id="crRepoStatus" role="status"></div>
  </div>
</div>

<details style="margin-bottom:14px">
  <summary style="cursor:pointer;font-size:12.5px;font-weight:600;color:var(--accent)">高级参数(一般不用改)</summary>
  <div class="section" style="margin-top:10px">
    <div class="row3">
      <div><label>单任务并发</label><input type="number" id="crConcurrency" min="1" max="32" value="4"></div>
      <div><label>单任务超时(分钟)</label><input type="number" id="crTimeout" min="2" max="240" value="30"></div>
      <div><label>单次 token 预算</label><input type="number" id="crBudget" min="0" step="100000" value="2000000"></div>
    </div>
    <div class="row3" style="margin-top:10px">
      <div><label>全局并发任务</label><input type="number" id="crParallel" min="1" max="4" value="2"></div>
      <div><label>记录保留天数</label><input type="number" id="crRetention" min="1" max="3650" value="90"></div>
      <div><label>结果推送</label><select id="crNotifyOn"><option value="always">每次推送</option><option value="failure">仅失败时</option><option value="never">不推送</option></select></div>
    </div>
    <div class="row3" style="margin-top:10px">
      <div><label>每仓库保留运行数</label><input type="number" id="crKeepRuns" min="1" max="1000" value="50"></div>
      <div><label>单仓库磁盘上限(MB)</label><input type="number" id="crDiskLimit" min="64" max="102400" value="2048"></div>
      <div><label>当日 token 预算 <span class="lbl-hint">(0=不限;所有仓库合计)</span></label><input type="number" id="crDailyBudget" min="0" step="100000" value="0"></div>
    </div>
    <div class="note" style="margin-top:10px">定时扫描每 60 秒一次,按北京时间判定:固定间隔看「距上次评审是否满 N 小时」,每天定时看「今天到点且今天还没评过(手动跑过也算)」。当日 token 预算用尽后,当天不再自动触发。</div>
    <div class="row" style="margin-top:10px">
      <div><label>排除路径 <span class="lbl-hint">(逗号分隔)</span></label><input type="text" id="crExclude" placeholder="**/node_modules/**,**/dist/**"></div>
      <div><label>OCR 可执行文件 <span class="lbl-hint">(留空 = 取 PATH)</span></label><input type="text" id="crOcrPath" placeholder="ocr"></div>
    </div>
    <div class="note" style="margin-top:10px">评审请求固定走本网关(端点由所选方案推导,不允许指向外部地址);代码片段会随评审意见入库,可用下面的开关关闭。</div>
    <label class="checkline"><input type="checkbox" id="crStoreComments" class="cbx"><span>保存评审意见里的代码片段(关闭则只存统计)</span></label>
  </div>
</details>

<div class="section">
  <strong style="font-size:13px">成员与可见范围</strong>
  <div class="note" style="margin-top:6px">谁负责哪个仓库,就在**那个仓库**的成员名单里勾谁(点仓库的「编辑」)。列在某个仓库成员里的人 = 能触发这个仓库的评审 + 能在自己的「我的用量」页看到它的评审结果。不在名单里的人既触发不了、也看不到。<br>触发还要求仓库打开「允许通过 API / MCP 触发」—— 成员名单决定「谁能」,那个开关决定「能不能从外部触发」。</div>
</div>

<div class="section">
  <strong style="font-size:13px">Webhook 自动评审</strong>
  <div class="note" style="margin-top:6px">在 GitLab / GitHub / Gitee 的项目 Webhook 里把回调指向下面的地址、事件勾「Push」,有人推代码就会自动评审。密钥全局一个:GitLab / Gitee 填进平台的「Token / 密码」框,GitHub 填进「Secret」(它会自动做签名校验)。只有仓库自己勾了「push 自动评审」、且推的正是该仓库配置的分支,才会真的触发。</div>
  <div class="row" style="margin-top:10px">
    <div><label>回调地址 <span class="lbl-hint">(只读,复制到托管平台)</span></label><input type="text" id="crWebhookUrl" readonly style="font-family:var(--font-mono);font-size:11px"></div>
    <div><label>密钥 <span class="lbl-hint">(自动回填,可直接复制;点「生成」换新密钥,保存后需同步更新托管平台)</span></label>
      <div style="display:flex;gap:6px">
        <input type="text" id="crWebhookSecret" placeholder="未设置 —— 点「生成」并保存后 Webhook 才启用" autocomplete="off" style="flex:1;font-family:var(--font-mono);font-size:11px">
        <button type="button" class="btn btn-outline btn-sm" onclick="copyWebhookSecret(this)">复制</button>
        <button type="button" class="btn btn-outline btn-sm" onclick="genWebhookSecret()">生成</button>
      </div>
    </div>
  </div>
  <div class="row" style="margin-top:10px">
    <div><label>防抖(秒) <span class="lbl-hint">同一仓库窗口内多次 push 只评一次,0=不防抖</span></label><input type="number" id="crWebhookDebounce" min="0" max="3600" step="30" value="300"></div>
    <div></div>
  </div>
  <div class="note" style="margin-top:8px">注意:托管平台那台机器必须能访问到这里的地址(容器部署别填 127.0.0.1);平台「测试/Test」按钮返回 2xx 即通。</div>
</div>

<div class="section">
  <strong style="font-size:13px">维护</strong>
  <div class="note" style="margin-top:6px">历史运行按「保留天数 + 每仓库保留条数」自动清理(默认每天一次);工作区是拉下来的代码副本,删掉只是下次评审要重新 clone。</div>
  <div class="row" style="margin-top:10px">
    <div>
      <label>立即清理历史 <span class="lbl-hint">(按上面的保留设置,不影响游标)</span></label>
      <div style="display:flex;align-items:center;gap:8px;margin-top:6px">
        <input type="text" id="crWorkspaceInfo" readonly placeholder="工作区占用 —">
        <button type="button" class="btn btn-outline btn-sm" onclick="pruneReviewHistory()">立即清理</button>
      </div>
    </div>
    <div>
      <label>清空评审数据 <span class="lbl-hint">(破坏性:统计+意见+游标全清,需管理员密码)</span></label>
      <div style="display:flex;align-items:center;gap:8px;margin-top:6px">
        <input type="password" id="crClearPassword" placeholder="管理员密码" autocomplete="current-password">
        <button type="button" class="btn btn-outline btn-sm" onclick="clearReviewData(false)">清空数据</button>
        <button type="button" class="btn btn-outline btn-sm" onclick="clearReviewData(true)">连工作区一起删</button>
      </div>
    </div>
  </div>
  <div class="inline-status" id="crMaintStatus" role="status"></div>
</div>

<div style="display:flex;justify-content:flex-end;gap:8px;margin:4px 0 24px">
  <button type="button" class="btn btn-primary" onclick="saveCodeReviewSettings()">保存代码评审设置</button>
</div>
</div>

<div id="planScheduleView" hidden aria-hidden="true">
<h2>方案组调度 <span style="font-size:12px;color:var(--dim);font-weight:400">预先排列多套方案组，按「星期几 + 时段」自动切换优先级；一个协议一套规则表，互不影响</span></h2>

${(() => {
  // 两个协议各一块：规则本来就按协议分桶、不跨协议排序，上下两块正好把这个事实画出来。
  // 用 data-sched-proto 而不是 data-proto —— switchProtoTab 会全局 toggle 所有 [data-proto]，
  // 借用它会让本视图的显示状态被侧栏标签条接管。
  const block = (proto, label, hint) => `<div class="section" data-sched-proto="${proto}" style="margin-bottom:16px">
  <div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap;margin-bottom:4px">
    <b style="font-size:14px">${label}</b>
    <span class="note" style="margin:0">${hint}</span>
  </div>
  <div id="schedActive-${proto}" role="status" style="font-size:12px;line-height:1.7;margin:10px 0;padding:9px 11px;border:1px solid var(--border);border-radius:5px;background:var(--surface)">正在加载调度状态…</div>
  <div id="schedOverridePick-${proto}" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px;font-size:12px"></div>
  <div id="schedOverride-${proto}" style="display:none;font-size:12px;line-height:1.6;margin-bottom:10px;padding:9px 11px;border:1px solid #eadfc3;border-radius:5px;background:#faf5e6;color:var(--orange);align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
    <span id="schedOverrideText-${proto}"></span>
    <button type="button" class="btn btn-outline btn-sm" onclick="scheduleClearOverride('${proto}')">取消手动指定</button>
  </div>
  <div id="schedHealth-${proto}" style="font-size:12px;line-height:1.7;margin-bottom:10px"></div>
  <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin:14px 0 6px">
    <b style="font-size:12px">方案组 <span class="note" style="margin:0;font-weight:400">列表顺序即优先级，第一条为组头</span></b>
    <button type="button" class="btn btn-outline btn-sm" onclick="scheduleToggleAddGroup('${proto}')">＋ 新建方案组</button>
  </div>
  <div id="schedGroupCreate-${proto}" style="margin-bottom:8px"></div>
  <div id="schedGroups-${proto}"></div>
  <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin:16px 0 6px">
    <b style="font-size:12px">时间规则 <span class="note" style="margin:0;font-weight:400">自上而下匹配，第一条命中的生效</span></b>
    <button type="button" class="btn btn-outline btn-sm" onclick="scheduleAddRule('${proto}')">＋ 新增规则</button>
  </div>
  <div id="schedRules-${proto}"></div>
</div>`;
  return block("anthropic", "Anthropic 调度", "入口 /v1/messages")
    + block("responses", "OpenAI 调度", "入口 /v1/responses");
})()}
<div class="note" style="margin-top:4px">调度只改变路由顺序，不改动任何方案自身配置，也无需重载。未命中任何规则时使用侧栏基础组的当前顺序，行为与未配置时完全一致。时间一律按北京时间判定；规则表为空时手动指定会在当日 24:00 收回。注意：方案卡片上的「默认入口」徽章表示侧栏基础组的组头，调度规则生效期间它不参与路由。</div>
</div>
...
<div id="userManageView" hidden aria-hidden="true">
<div class="view-intro">
  <h2>用户管理</h2>
  <p>虚拟 Key 全局唯一，点 key 可复制；失效时间留空 = 永不过期。全局禁用后该用户在所有方案不可用（表格中灰色显示）；方案真实 Key 必填，用户才能使用该方案。保存前改动只在本页生效。</p>
</div>
<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px">
  <button type="button" class="btn btn-outline btn-sm" onclick="addGlobalUser()">添加用户</button>
  <span class="note" style="margin:0">虚拟Key自动生成（jx-开头24位随机码）。</span>
</div>
...
<div style="display:flex;justify-content:flex-end;gap:8px;margin:16px 0 8px">
<button type="button" class="btn btn-primary" onclick="saveUsers()">保存全部</button>
</div>
</div>

<div id="notifyManageView" hidden aria-hidden="true">
<div class="view-intro">
  <h2>通知管理 <span style="font-size:12px;color:var(--dim);font-weight:400">创建 Markdown 站内信，发布到成员「我的用量」页的消息铃铛</span></h2>
  <p>新建后先保存为草稿，点「发布」才会推送给成员；已发布的可撤回为草稿或删除。目标选「指定成员」时只有被选中的人在铃铛里看到这条通知。</p>
</div>
<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px">
  <button type="button" class="btn btn-primary btn-sm" onclick="notifyOpenEditor()">＋ 新建通知</button>
  <button type="button" class="btn btn-outline btn-sm" onclick="loadNotifications(true)">刷新</button>
  <span class="inline-status" id="notifyStatus" role="status"></span>
</div>
<div class="section" id="notifyEditor" hidden>
  <input type="hidden" id="notifyEditId" value="">
  <div class="row">
    <div><label>标题<span class="req">*</span> <span class="lbl-hint">(最多 100 字)</span></label><input type="text" id="notifyTitle" maxlength="100" placeholder="一句话说明这条通知的事"></div>
    <div><label>接收对象</label>
      <select id="notifyTargetType" onchange="notifyTargetTypeChanged()">
        <option value="all">全员</option>
        <option value="user">指定成员</option>
      </select>
    </div>
  </div>
  <div id="notifyTargetUsers" style="display:none;margin-top:10px">
    <label>选择成员<span class="req">*</span></label>
    <div id="notifyUserChecks" class="notify-user-checks"></div>
    <div class="note">可多选；后续要改名单需先撤回再编辑。</div>
  </div>
  <label style="margin-top:12px">正文（Markdown）<span class="req">*</span></label>
  <textarea id="notifyContent" rows="10" oninput="notifyPreview()" placeholder="支持 Markdown：标题、加粗、列表、代码块、链接、表格等" style="font-family:var(--font-body)"></textarea>
  <div style="margin-top:10px">
    <label style="font-size:12px;font-weight:650">预览 <span style="color:var(--dim);font-weight:400">(成员看到的渲染效果)</span></label>
    <div class="md-preview" id="notifyPreviewBox"></div>
  </div>
  <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px">
    <button type="button" class="btn btn-outline btn-sm" onclick="notifyCloseEditor()">取消</button>
    <button type="button" class="btn btn-primary btn-sm" onclick="notifySaveDraft()">保存草稿</button>
  </div>
</div>
<div class="section" style="padding:0;overflow-x:auto">
  <table>
    <thead><tr><th>标题</th><th style="width:130px">目标</th><th style="width:80px">状态</th><th style="width:160px">发布时间</th><th style="width:250px">操作</th></tr></thead>
    <tbody id="notifyBody"><tr><td colspan="5" style="color:var(--dim);text-align:center;padding:18px">打开本页时自动加载</td></tr></tbody>
  </table>
</div>
</div>
</div>
...
<div class="modal-overlay" id="notifyViewModal">
<div class="modal" style="max-width:640px">
<div class="modal-hd"><h3 id="notifyViewTitle">通知</h3><button class="modal-close" onclick="closeNotifyView()">关闭</button></div>
<div class="modal-body">
<div style="font-size:11px;color:var(--dim);margin-bottom:10px" id="notifyViewMeta"></div>
<div class="md-preview" id="notifyViewContent"></div>
</div>
</div>
</div>
...
<div class="modal-body">
<label>粘贴方案代码（可选）</label>
<textarea id="profileCodePaste" rows="3" style="resize:vertical" oninput="applyProfileCode()" placeholder="把另一套环境里「复制代码」得到的 JSON 粘到这里，下面各项自动填充；不粘就直接手填新建"></textarea>
<div class="inline-status" id="profileCodeStatus" role="status"></div>
<div style="display:flex;justify-content:flex-end"><button type="button" class="btn btn-outline btn-sm" id="profileCodeClear" style="display:none" onclick="clearProfileCode()">清除代码</button></div>
<div class="note">方案代码含该方案的全部配置（协议、上游、模型别名、峰谷时段、各类配额倍率、上下文窗口等），但不含用户分配与额度池。导入后需自行分配真实Key，并把方案加入故障转移分组。</div>
<div class="row">
...
<div class="note">模型别名在创建后进入方案编辑页配置（允许模型由别名目标自动生成，无需手填）。别名与允许模型列表都为空时会放行所有模型名。</div>
<div style="margin-top:16px;display:flex;justify-content:flex-end;gap:8px">
<button type="button" class="btn btn-outline btn-sm" onclick="closeProfileModal()">取消</button>
<button type="button" class="btn btn-primary btn-sm" id="createProfileBtn" onclick="createProfile()">创建方案</button>
</div>
</div>
</div>
</div>
...
<div class="modal" style="max-width:640px">
<div class="modal-hd"><h3>方案代码</h3><button class="modal-close" onclick="closeCodeModal()">关闭</button></div>
<div class="modal-body">
<div class="note" id="codeModalNote">这段 JSON 可以粘贴到另一套环境的「新增方案」里，快速重建同名方案。不含用户分配与额度池。</div>
<textarea id="codeModalText" rows="14" readonly spellcheck="false" style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:1.5" onclick="this.select()"></textarea>
<div style="margin-top:12px;display:flex;justify-content:flex-end;gap:8px">
<button type="button" class="btn btn-outline btn-sm" onclick="closeCodeModal()">关闭</button>
<button type="button" class="btn btn-primary btn-sm" onclick="copyCodeModalText(this)">复制</button>
</div>
</div>
</div>
</div>
...
<script src="${assets.url("md.js")}"><\/script>
<script>
...
// 下发给页面的通知配置:SMTP 密码**明文下发**(应管理员要求,密码框显示为明文可核对;
// 与 webhook 密钥同一逻辑 —— 这是管理员自己要核对/粘贴的运维凭据,设置页本身有密码门)。
// hasSmtpPass 保留给「是否已配置」的提示用。
const NOTIFIER_CFG=${JSON.stringify((() => {
  const n = { ...(config.notifier || {}) };
  const hasSmtpPass = !!n.smtpPass;
  return { ...n, hasSmtpPass };
})()).replace(/</g, "\\x3c")};
let INITIAL_REVIEW=${JSON.stringify(getPublicSettings().codeReview || {}).replace(/</g, "\\x3c")};
...
export function dashboardHtml(d) {
  const { assets } = d;
  // 侧栏图标:与「我的用量」同款范式(24 网格缩到 18px、描边 2.5 —— 这组数是配套的,
  // 改 svg 尺寸就得同步改描边与 CSS 里的过渡注释)。
  const dIco = (paths) => `<svg class="nav-ico" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
  const DICO = {
    overview: dIco('<rect x="3.5" y="3.5" width="7" height="7" rx="2"/><rect x="13.5" y="3.5" width="7" height="7" rx="2"/><rect x="3.5" y="13.5" width="7" height="7" rx="2"/><rect x="13.5" y="13.5" width="7" height="7" rx="2"/>'),
    users: dIco('<circle cx="12" cy="8" r="3.5"/><path d="M5 20c.8-3.4 3.6-5 7-5s6.2 1.6 7 5"/>'),
    clients: dIco('<rect x="3" y="4.5" width="18" height="12.5" rx="2"/><path d="M9 20.5h6M12 17v3.5"/>'),
    detail: dIco('<path d="M8 6h12M8 12h12M8 18h12"/><circle cx="4.2" cy="6" r="1.1" fill="currentColor" stroke="none"/><circle cx="4.2" cy="12" r="1.1" fill="currentColor" stroke="none"/><circle cx="4.2" cy="18" r="1.1" fill="currentColor" stroke="none"/>'),
    profiles: dIco('<path d="M12 3.5 20 8l-8 4.5L4 8Z"/><path d="M4 12.5 12 17l8-4.5"/><path d="M4 17 12 21.5 20 17"/>'),
    rates: dIco('<path d="M19 5 5 19"/><circle cx="7" cy="7" r="2.2"/><circle cx="17" cy="17" r="2.2"/>'),
    errors: dIco('<path d="M12 3.5 21 19.5H3Z"/><path d="M12 9.5v4.5"/><circle cx="12" cy="16.8" r="1" fill="currentColor" stroke="none"/>'),
    production: dIco('<path d="M4 4v16h16"/><path d="M7.5 15.5 11 11.5l3 2.5 5-6"/>'),
    sessions: dIco('<path d="M7.5 4h9a4 4 0 0 1 4 4v3a4 4 0 0 1-4 4h-9a4 4 0 0 1-4-4v-3a4 4 0 0 1 4-4Z"/><path d="M8.5 15h5L11 20Z" fill="currentColor" stroke="none"/>'),
    costs: dIco('<circle cx="12" cy="12" r="8.5"/><path d="M12 7v10M15 9.2c-.7-1-1.8-1.4-3-1.4-1.7 0-3 .9-3 2.1 0 2.8 6 1.7 6 4.4 0 1.2-1.3 2.1-3 2.1-1.2 0-2.3-.4-3-1.4"/>'),
    review: dIco('<rect x="4" y="3.5" width="16" height="17" rx="2.5"/><path d="M8 9h8"/><path d="M8 13h4"/><path d="M8 16.5h8"/>'),
    settings: dIco('<circle cx="12" cy="12" r="3"/><path d="M12 3.5v2.2M12 18.3v2.2M4.9 4.9l1.6 1.6M17.5 17.5l1.6 1.6M3.5 12h2.2M18.3 12h2.2M4.9 19.1l1.6-1.6M17.5 6.5l1.6-1.6"/>'),
    docs: dIco('<path d="M5 4.5h9l5 5V19.5H5Z"/><path d="M14 4.5v5h5"/><path d="M8.5 13h7M8.5 16h7"/>'),
    logout: dIco('<path d="M14 4.5H6.5A1.5 1.5 0 0 0 5 6v12a1.5 1.5 0 0 0 1.5 1.5H14"/><path d="M10.5 12H20M16.5 8.5 20 12l-3.5 3.5"/>'),
  };
  return `<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2096%2096%22%3E%3Crect%20width%3D%2296%22%20height%3D%2296%22%20rx%3D%2222%22%20fill%3D%22%232f6e50%22%2F%3E%3Cg%20fill%3D%22none%22%20stroke%3D%22%23fbfbf8%22%20stroke-width%3D%2213%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20transform%3D%22translate(48%2048)%20scale(0.88)%20translate(-48%20-48)%22%3E%3Cpath%20d%3D%22M37%2026.5H31.5Q20.5%2026.5%2020.5%2037.5V58.5Q20.5%2069.5%2031.5%2069.5H37%22%2F%3E%3Cpath%20d%3D%22M59%2026.5H64.5Q75.5%2026.5%2075.5%2037.5V58.5Q75.5%2069.5%2064.5%2069.5H59%22%2F%3E%3C%2Fg%3E%3Ccircle%20cx%3D%2248%22%20cy%3D%2248%22%20r%3D%226.2%22%20fill%3D%22%23fbfbf8%22%2F%3E%3C%2Fsvg%3E">
...
<aside class="dash-side">
  <div class="sidebar-hd">
    <div class="sidebar-brand"><svg class="brand-logo" viewBox="0 0 96 96" aria-hidden="true"><rect width="96" height="96" rx="22" fill="#2f6e50"/><g fill="none" stroke="#fbfbf8" stroke-width="11" stroke-linecap="round" stroke-linejoin="round" transform="translate(48 48) scale(0.9) translate(-48 -48)"><path d="M37 26.5H31.5Q20.5 26.5 20.5 37.5V58.5Q20.5 69.5 31.5 69.5H37"/><path d="M59 26.5H64.5Q75.5 26.5 75.5 37.5V58.5Q75.5 69.5 64.5 69.5H59"/></g><circle cx="48" cy="48" r="4.95" fill="#fbfbf8"/></svg><div><h1>团队用量</h1><div class="command-status"><span class="led on"></span>监控服务运行中</div></div></div>
    <span class="meta" id="meta">正在加载数据</span>
  </div>
  <nav class="sidebar-nav" id="dashNav" role="tablist" aria-label="功能菜单">
    <button type="button" class="nav-btn active" role="tab" id="workspace-tab-overview" aria-controls="workspace-panel-overview" aria-selected="true" tabindex="0" data-section="overview">${DICO.overview}数据总览</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-users" aria-controls="workspace-panel-users" aria-selected="false" tabindex="-1" data-section="users">${DICO.users}用户用量</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-clients" aria-controls="workspace-panel-clients" aria-selected="false" tabindex="-1" data-section="clients">${DICO.clients}客户端用量</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-detail" aria-controls="workspace-panel-detail" aria-selected="false" tabindex="-1" data-section="detail">${DICO.detail}明细记录</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-profiles" aria-controls="workspace-panel-profiles" aria-selected="false" tabindex="-1" data-section="profiles">${DICO.profiles}方案中心</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-rates" aria-controls="workspace-panel-rates" aria-selected="false" tabindex="-1" data-section="rates">${DICO.rates}配额倍率</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-errors" aria-controls="workspace-panel-errors" aria-selected="false" tabindex="-1" data-section="errors">${DICO.errors}错误记录</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-production" aria-controls="workspace-panel-production" aria-selected="false" tabindex="-1" data-section="production">${DICO.production}产出质量</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-sessions" aria-controls="workspace-panel-sessions" aria-selected="false" tabindex="-1" data-section="sessions">${DICO.sessions}会话使用情况</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-costs" aria-controls="workspace-panel-costs" aria-selected="false" tabindex="-1" data-section="costs">${DICO.costs}等值成本</button>
    <button type="button" class="nav-btn" role="tab" id="workspace-tab-review" aria-controls="workspace-panel-review" aria-selected="false" tabindex="-1" data-section="review">${DICO.review}代码评审</button>
  </nav>
  <div class="dash-side-ft">
    <a class="nav-btn" href="/settings">${DICO.settings}设置</a>
    <a class="nav-btn" href="/wiki/" target="_blank">${DICO.docs}文档</a>
    <button type="button" class="nav-btn danger" onclick="fetch('/api/logout',{method:'POST',headers:{'x-csrf-token':(document.cookie.match(/tm_csrf=([^;]+)/)||[])[1]||''}}).then(()=>toastThen('已退出登录',()=>location.reload()))">${DICO.logout}退出</button>
  </div>
</aside>
<main class="dash-main">
  <div class="toolbar"><div class="controls"><select id="profileSel" aria-label="查看方案" onchange="switchProfileView(this.value)"><option value="">全部方案</option></select><button id="autoRefreshBtn" class="ar-on">自动刷新：开</button></div></div>
  <section id="workspace-panel-overview" class="workspace-panel pane active" role="tabpanel" aria-labelledby="workspace-tab-overview">
...
<section class="chart-filters" aria-label="图表筛选">
  ...
  <span class="chart-filter-hint" id="chartFilterHint">24 小时图仅统计单日（选「按日」或将开始/结束日期设为同一天）；模型/用户筛选对部分图表不适用；按模型筛选时不含缓存 Token</span>
</section>
<section class="chart-workspace" aria-label="用量图表">
  ...
  <div class="chart-panel chart-users"><div class="chart-head"><h2 id="pieTitle">用户分布</h2><div class="proto-seg" id="pieDim" role="group" aria-label="分布维度"><button type="button" class="on" data-dim="user">按用户</button><button type="button" data-dim="client">按客户端</button></div></div><div class="chart-canvas"><canvas id="pie"></canvas></div></div>
  <div class="chart-panel chart-models"><div class="chart-head"><h2>模型请求分布</h2><div class="proto-seg" id="modelDim" role="group" aria-label="统计维度"><button type="button" class="on" data-dim="model">按模型</button><button type="button" data-dim="profile">按方案</button></div></div><div class="chart-canvas"><canvas id="modelChart"></canvas></div></div>
  <div class="chart-panel chart-hourly"><div class="chart-head"><h2>24 小时趋势</h2><span class="chart-note" id="hourNote"></span></div><div class="chart-canvas"><canvas id="hourChart"></canvas></div></div>
  <div class="chart-panel chart-hmodel"><div class="chart-head"><h2>24 小时模型使用趋势</h2><span class="chart-note" id="hourModelNote"></span></div><div class="chart-canvas"><canvas id="hourModelChart"></canvas></div></div>
  <div class="chart-panel chart-profile"><div class="chart-head"><h2>方案请求情况</h2><span class="chart-note" id="profileNote"></span></div><div class="chart-canvas"><canvas id="profileChart"></canvas></div></div>
    ...
    <section id="workspace-panel-users" role="tabpanel" aria-labelledby="workspace-tab-users" class="workspace-panel pane" hidden><div class="workspace-panel-inner">
    ...
    <section id="workspace-panel-clients" role="tabpanel" aria-labelledby="workspace-tab-clients" class="workspace-panel pane" hidden><div class="workspace-panel-inner">
      <div class="workspace-panel-head"><strong>客户端用量</strong><span class="workspace-panel-summary" id="clientContext"></span></div>
      <div class="workspace-panel-scroll">
        <div class="note" style="padding:6px 12px;margin-top:0;text-align:left;font-size:11px;color:var(--dim)">客户端依据请求头（User-Agent / X-App / Originator）判定，可被伪造，仅供参考；未识别的请求单列一行。客户端与协议是两个维度 —— 同一个客户端可以走不同协议，同一个协议也服务多个客户端。受模型筛选影响时不适用（该表没有模型维度）。</div>
        <table id="clientBoardTable"><thead><tr><th>客户端</th><th>用户</th><th class="n">请求数</th><th class="n">输入</th><th class="n">输出</th><th class="n">合计</th><th class="n">占比</th></tr></thead><tbody id="clientBoardBody"></tbody></table>
      </div>
    </div></section>
    <section id="workspace-panel-detail" role="tabpanel" aria-labelledby="workspace-tab-detail" class="workspace-panel pane" hidden><div id="detailSec">
    ...
    <section id="workspace-panel-profiles" role="tabpanel" aria-labelledby="workspace-tab-profiles" class="workspace-panel pane" hidden><div id="profileSummarySec" class="workspace-panel-inner">
      <div class="workspace-panel-head"><strong>方案中心</strong><span class="workspace-panel-summary" id="profileContext">当前查看：全部方案</span></div><div class="workspace-panel-scroll"><table><thead><tr><th>方案</th><th>入口</th><th>上游</th><th class="n">今日请求</th><th class="n">今日用量</th><th>使用状态</th><th>状态</th></tr></thead><tbody id="profileSummaryBody"></tbody></table></div>
    </div></section>
    <section id="workspace-panel-rates" role="tabpanel" aria-labelledby="workspace-tab-rates" class="workspace-panel pane" hidden><div class="workspace-panel-inner">
    ...
    <section id="workspace-panel-errors" role="tabpanel" aria-labelledby="workspace-tab-errors" class="workspace-panel pane" hidden><div id="errorSec">
    ...
    <section id="workspace-panel-production" role="tabpanel" aria-labelledby="workspace-tab-production" class="workspace-panel pane" hidden><div class="workspace-panel-inner">
    ...
    <section id="workspace-panel-costs" role="tabpanel" aria-labelledby="workspace-tab-costs" class="workspace-panel pane" hidden><div class="workspace-panel-inner">
    ...
    <section id="workspace-panel-sessions" role="tabpanel" aria-labelledby="workspace-tab-sessions" class="workspace-panel pane" hidden><div class="workspace-panel-inner">
      <div class="workspace-panel-head"><strong>会话使用情况</strong><select id="sessRangeSel" onchange="loadSessions()"><option value="today">今日</option><option value="7d" selected>近7天</option><option value="30d">近30天</option></select><button type="button" class="q-focus-btn" id="sessDrillClear" style="display:none" onclick="clearSessDrill()">清除下钻</button><span class="workspace-panel-summary" id="sessSummary"></span></div>
      <div class="workspace-panel-scroll">
        <div class="note" style="padding:6px 12px;margin-top:0;text-align:left;font-size:11px;color:var(--dim)">会话是使用习惯的最小单位:长会话连续用缓存命中高,频繁新开则缓存反复重建。点上方某一行可下钻到该成员的会话流水。token 与缓存率只从采集上线后算起,更早的会话只有工具数据。</div>
        <table id="sessUserTable"><thead><tr><th>成员</th><th class="n">会话</th><th class="n">轮数</th><th class="n">Token</th><th class="n">缓存率</th><th class="n">单轮新增</th><th class="n">失败率</th><th class="n">碎片</th><th>分档</th></tr></thead><tbody></tbody></table>
        <div class="workspace-panel-head" style="border-top:1px solid var(--border)"><strong>会话流水</strong><span id="sessDrillHint" style="font-size:11px;color:var(--dim);font-weight:400"></span></div>
        <table id="sessTable"><thead><tr><th>成员</th><th>客户端</th><th>项目</th><th>会话</th><th>时段</th><th class="n">轮数</th><th class="n">Token</th><th class="n">缓存率</th><th class="n">单轮新增</th><th class="n">工具</th><th class="n">文件</th><th class="n">净产出</th><th class="n">失败率</th><th>分档</th></tr></thead><tbody></tbody></table>
        <div id="sessNote" class="note" style="padding:4px 12px;font-size:11px;color:var(--dim);text-align:left"></div>
      </div>
    </div></section>
    <section id="workspace-panel-review" role="tabpanel" aria-labelledby="workspace-tab-review" class="workspace-panel pane" hidden><div class="workspace-panel-inner">
      <div class="workspace-panel-head"><strong>代码评审</strong><span class="workspace-panel-summary" id="crStatusLine"></span></div>
      <div id="crPanelBody"><div class="lb-msg" style="padding:16px">加载中…</div></div>
    </div></section>
    ${reviewDetailDialog("crDetailDlg", `<a class="pager-btn" id="crDetailExport" href="#" style="display:none">导出报告</a><button type="button" class="pager-btn" id="crDetailNotify" style="display:none">发送给负责人</button><button type="button" class="pager-btn" id="crDetailResolve" style="display:none">标记已解决</button>`)}

</main>
</div>
<script src="${assets.url("ui.js")}"><\/script>
<script src="${assets.url("dashboard.js")}"><\/script>
<script src="${assets.url("code-review.js")}"><\/script>
</body></html>`;
}
...
// ── 接入指南的公共骨架 ────────────────────────────────────────────────────────
// 「配置 Claude Code」与「配置 Codex」共用形状、各自填文案。三条规矩（与
// public/assets/setup-guide.js 的注释同源）：
// 1. 块内不写 id，一律 data-role —— 同一页会同时存在两个引导块，重复 id 会让
//    getElementById 静默拿到另一个面板的元素；
// 2. 没有内联 onclick/oninput，交互由脚本按 root 事件委托；
// 3. 面板用 data-panel 而不是 id，切 tab 时只动自己这一块。
function setupHostRowHtml(hint) {
  return `<div class="host-row"><span>服务器地址：</span><span style="color:var(--dim)" data-role="schemeLabel">http://</span><input data-role="hostInput" value="" spellcheck="false"><code>自动取自当前访问地址（含 https），可修改</code>${hint || ""}</div>`;
}
function setupTabsHtml() {
  return `<div class="tabs"><button class="on" data-tab="script">一键脚本</button><button data-tab="manual">手动配置</button><button data-tab="ccswitch">cc-switch 用户</button></div>`;
}
function setupPreHtml(role, extraStyle) {
  return `<pre${extraStyle ? ` style="${extraStyle}"` : ""}><button class="copy-btn" data-copy>复制</button><code data-role="${role}"></code></pre>`;
}
// cc-switch 一键导入块：两个引导的第三种方式共用（app 与载荷由客户端按 guide 推导）。
function setupCcSwitchBoxHtml() {
  return `<div class="box"><h3>一键导入 cc-switch</h3>
    <ol>
      <li>确认本机已装 cc-switch（它是桌面应用，与浏览器无关）</li>
      <li>点下面的按钮，浏览器会唤起 cc-switch 并弹出确认框</li>
      <li>确认后供应商列表里会出现名为 <code>CC Team</code> 的一项，切换过去即可</li>
    </ol>
    <a class="cc-import-btn" data-role="ccLink" href="#">在 cc-switch 中打开</a>
    <div class="note">按钮没反应时（部分浏览器不允许从页面唤起本机应用），复制下面这条链接：</div>
    ${setupPreHtml("ccLinkText", "max-height:110px;overflow:auto")}
    <div class="note">重复导入会再生成一个同名供应商，导入前先把旧的删掉。</div>
    <div class="warn">网关地址是内网地址（192.168. / 10. / 127.）时，cc-switch 的确认框会提示「私有端点」风险——那是它的风险分类器在提醒数据要发往内网地址，本网关本来就在内网，确认即可。</div>
  </div>`;
}

// 「配置 Codex」主体:状态 banner + host 行 + 三个方式 tab/panel。
// /setup 独立页(codexSetupHtml)与我的用量页内分区(personalUsageHtml)共用;
// 两个宿主页面不会同屏出现两个同 kind 的块,但同页可能有 Claude Code 那一个,
// 所以块内一律 data-role / data-panel（见上面的公共骨架注释）。
function codexSetupBodyHtml(cat, banner) {
  return `<div class="setup-guide" data-guide="codex">${banner}
${setupHostRowHtml(cat.entries.length ? ` <code>可用模型：${cat.entries.map(e => e.slug).join(" / ")}（来自方案配置的别名）</code>` : "")}
${setupTabsHtml()}
<div class="panel on" data-panel="script">
  <div class="box"><h3>macOS / Linux — 在「终端」执行</h3>
    ${setupPreHtml("curlCmd")}
  </div>
  ...
  <div class="box"><h3>执行后</h3>
    ...
    <div class="warn">脚本会把顶层 <code>openai_base_url</code> 指向本网关，并同步 <code>~/.codex/auth.json</code> 的 <code>OPENAI_API_KEY</code> 为你的虚拟 Key（原内容自动备份）——这样 Codex 的 remote compact（会话压缩）也走网关，压缩的 token 进配额与统计。覆盖了你的 ChatGPT 登录态时，从备份复制回 <code>auth.json</code> 即可恢复。</div>
  </div>
</div>
<div class="panel" data-panel="manual">
  <div class="box"><h3>① 追加到 ~/.codex/config.toml 顶部（若已有同名键则替换）</h3>
    ${setupPreHtml("tomlBlock", "max-height:300px;overflow:auto")}
    <div class="warn">请确认 <code>~/.codex/auth.json</code> 的 <code>OPENAI_API_KEY</code> 是你的虚拟 Key（一键脚本会自动写入）——remote compact（会话压缩）走内置通道的 WebSocket 协议，凭据与地址缺一不可，配好后压缩也走网关并计入配额。</div>
  </div>
  <div class="box"><h3>② 另存为 ~/.codex/models.json</h3>
    ${setupPreHtml("modelsJson", "max-height:300px;overflow:auto")}
    <div class="note">保存后完全退出 Codex（Cmd+Q）重新打开。</div>
  </div>
</div>
<div class="panel" data-panel="ccswitch">
  ${setupCcSwitchBoxHtml()}
  <div class="box"><h3>手动添加自定义供应商</h3>
    <ol>
      <li>cc-switch → Codex → Add Provider → 自定义</li>
      <li>把下面的 TOML 粘贴进供应商配置（name 可自定，字段保留）：</li>
    </ol>
    ${setupPreHtml("tomlBlock2", "max-height:260px;overflow:auto")}
...
// 「配置 Claude Code」主体。三种方式与 Codex 对齐，内容完全不同：
// 目标文件是 ~/.claude/settings.json 的 env 段（不是 config.toml），入口地址不带 /v1
// （Claude Code 自己接 /v1/messages）。hints 见 lib/claude-setup-script.mjs 的
// buildAnthropicSetupHints——推荐地址是按成员的方案归属算出来的，不是常量。
function claudeSetupBodyHtml(hints, banner) {
  const h = hints || {};
  const aliasList = (h.aliases || []).join(" / ");
  return `<div class="setup-guide" data-guide="claude">${banner}
${setupHostRowHtml(aliasList ? ` <code>可用模型别名：${aliasList}（来自方案配置的别名）</code>` : "")}
${setupTabsHtml()}
<div class="panel on" data-panel="script">
  <div class="box"><h3>macOS / Linux — 在「终端」执行</h3>
    ${setupPreHtml("curlCmd")}
  ...
  <div class="box"><h3>执行后</h3>
    <ol>
      <li>脚本先备份，再把两个键<strong>合并</strong>进 <code>~/.claude/settings.json</code> 的 <code>env</code> 段（Windows 为 <code>%USERPROFILE%\.claude\settings.json</code>）</li>
      <li><strong>重开一个终端（或重启 Claude Code）</strong>后即可使用</li>
    </ol>
    <div class="note">只改 <code>env</code> 里的 <code>ANTHROPIC_BASE_URL</code> 与 <code>ANTHROPIC_AUTH_TOKEN</code> 两个键，你已有的 permissions / hooks / statusLine / model 与其他 env 键全部保留；重复执行安全；结束时打印本次操作摘要与回滚方法。本机没有 node / python3 / jq 时，脚本会拒绝改动并直接打印需要手动粘贴的内容。</div>
    <div class="note">为什么写文件而不是让你 export：<strong>settings.json 的 env 段优先于 shell 环境变量</strong>——只要它存在，终端里 export 的 ANTHROPIC_* 就不生效。所以这里把文件放第一位，export 只作为临时手段写在「手动配置」里。</div>
  </div>
</div>
<div class="panel" data-panel="manual">
  <div class="box"><h3>① 把下面这两个键合并进 ~/.claude/settings.json 的 env 段</h3>
    ${setupPreHtml("settingsJson", "max-height:220px;overflow:auto")}
    <div class="note">没有 <code>env</code> 段就新建一段；文件里其余内容（permissions / hooks / statusLine / model 等）原样不动。当前地址：<code data-role="baseUrl"></code></div>
  </div>
  <div class="box"><h3>② 用哪个模型名</h3>
    <ol>
      <li>网关按「模型别名」路由：别名填进 <code>ANTHROPIC_MODEL</code>，网关会换成上游的真实模型。本 Key 可用的别名：<code data-role="modelHint"></code></li>
      <li>不设 <code>ANTHROPIC_MODEL</code> 时 Claude Code 用它自带的模型名，这需要管理员在方案里放行同名模型，否则会收到 403「Model … is not allowed」——<strong>最省事的做法是设成别名</strong>。</li>
      <li>（可选）后台小模型走同一套白名单，可以把它也设成一个别名：<code>ANTHROPIC_DEFAULT_HAIKU_MODEL</code>（旧版读 <code>ANTHROPIC_SMALL_FAST_MODEL</code>）。</li>
    </ol>
  </div>
  <div class="box"><h3>③ 只对当前终端生效（临时 / 排障用）</h3>
    ${setupPreHtml("shellExport")}
    <div class="warn">仅当前这个终端有效；而且当 <code>~/.claude/settings.json</code> 已经有 <code>env</code> 段时，文件里的值优先，这两行 export 不生效。</div>
  </div>
  <div class="box"><h3>④ 地址带不带方案后缀</h3>
    <ol>
      <li><strong>不带后缀</strong>（默认入口，就是上面那个地址）：网关在「默认方案组」里按顺序 failover，某个方案限流或熔断时自动换下一个——多数人用这个</li>
      <li><strong>带后缀</strong>：<code>http://主机:端口/方案后缀</code> 把 Claude Code 钉在某一个方案上（例如 <code>/glm</code>）</li>
      <li>注意：方案属于「默认方案组」时不允许带后缀直连（否则就绕开了 failover），这类方案只能用不带后缀的地址</li>
    </ol>
    ${h.note ? `<div class="warn">${escHtml(h.note)}</div>` : ""}
  </div>
</div>
<div class="panel" data-panel="ccswitch">
  ${setupCcSwitchBoxHtml()}
  <div class="box"><h3>手动添加自定义供应商</h3>
    <ol>
      <li>cc-switch → Claude → Add Provider → 自定义</li>
      <li>Base URL 填 <code data-role="ccEndpoint"></code>，API Key 填你的虚拟 Key</li>
    </ol>
    <div class="warn">注意：cc-switch 切换供应商时会重写 <code>~/.claude/settings.json</code>。请把你的 <code>permissions</code> / <code>hooks</code> / <code>statusLine</code> 放进它的「公共配置 / Shared Config Snippet」，否则切换后会丢。</div>
  </div>
</div></div>`;
}

// 「配置 MCP」主体:把网关的 MCP 端点(见 server.mjs handleMcpPost 上方注释)接进成员的
// Claude Code / Codex。与 claude/codex 两个引导共用 setup-guide 的骨架规矩(data-role、
// host 现算、无内联交互),但没有方式 tab——Claude Code 一条命令、Codex 一段配置,
// 没有第三种分发方式。工具一览/安全提示是静态文案,不依赖 host。
function mcpSetupBodyHtml(key, banner) {
  return `<div class="setup-guide" data-guide="mcp">${banner || ""}
${setupHostRowHtml()}
<div class="box"><h3>Claude Code — 一条命令接入</h3>
    <div class="note">在本机终端执行（Claude Code 已装好；虚拟 Key 就是你在本页的接入身份）：</div>
    ${setupPreHtml("mcpClaudeCmd")}
    <div class="note">执行后<strong>重开一个 Claude Code 会话</strong>生效；<code>claude mcp list</code> 可确认连接状态。删除：<code>claude mcp remove cc-team</code>。</div>
  </div>
  <div class="box"><h3>接入后 Claude 能帮你做什么</h3>
    <ol>
      <li><strong>my_quota</strong>：查各额度池余额（已用/上限/剩余/当前峰谷倍率/临时加量）——开工前看一眼，撞 429 后定位原因</li>
      <li><strong>price_list</strong>：当前时段各模型倍率价目表（rate 越低越便宜）——选模型前对比</li>
      <li><strong>check_in</strong>：每日签到领随机 token，连续天数不断</li>
      <li><strong>request_quota</strong>：向管理员提交加量申请</li>
      <li><strong>my_usage</strong> / <strong>leaderboard</strong>：查自己的用量明细 / 团队排行榜</li>
      <li><strong>code_review_list</strong> / <strong>code_review_findings</strong>：看你负责的仓库的评审结果与<strong>具体问题</strong>（文件、行号、描述、建议改法）——直接问「我这次改动评过没有」「哪些要改」，它会读给你听并帮你改</li>
      <li><strong>code_review_trigger</strong>：触发一次评审（<strong>写操作</strong>，真消耗 token；每 Key 60 秒一次）——评的是仓库里<strong>已提交</strong>的最新代码，不是本地未提交的改动</li>
      <li><strong>使用手册</strong>：本手册已挂成 MCP 资源，Claude 能直接回答「配额怎么算」「429 是什么意思」这类问题</li>
    </ol>
    <div class="note">评审类工具（code_review_*）要在管理员把你加进<strong>某个仓库的成员名单</strong>之后才会出现——没被授权时工具列表里根本看不到它们。详见<a href="/wiki/#/mcp" target="_blank" rel="noopener" style="color:var(--accent)">MCP 接入</a>与<a href="/wiki/#/code-review" target="_blank" rel="noopener" style="color:var(--accent)">代码评审</a>。</div>
  </div>
<div class="box"><h3>Codex — 在 ~/.codex/config.toml 里加一段</h3>
    ${setupPreHtml("mcpCodexToml", "max-height:160px;overflow:auto")}
    <div class="note">需要你的 Codex 版本支持 streamable HTTP 形式的 MCP（<code>url</code> 字段）；较老版本要加 <code>experimental_use_rmcp_client = true</code>，字段以你版本的官方文档为准。改完重启 Codex 会话生效。</div>
  </div>
<div class="box"><h3>安全提示</h3>
  <div class="warn">命令里带你的虚拟 Key——它就是你的身份（用量、签到、申请都以它记账）。不要把带 Key 的命令发到群里或提交进代码仓库；本页的命令只在你的浏览器里生成，复制走即可。</div>
</div>
</div>`;
}
...
${codexSetupBodyHtml(cat, banner)}
<script>
// 引导面板的数据注入。setup-guide.js 依赖 SETUP_GUIDES 这个全局（先注入、后加载）。
// 用 var 而不是 const：同一页被引用两次时 const 会抛重声明错。
// 注入前把 < 转义成 \\u003c —— 别名是管理员可控字符串，含 <\\/script> 时会直接截断脚本块。
// （这条注释里的斜杠也是转义的：HTML 解析器不认 JS 注释，正文里出现未转义的结束标签
//   会在那里把整个脚本块切掉，SETUP_GUIDES 于是一个字都没赋上——踩过一次。）
var SETUP_GUIDES=${JSON.stringify([{ guide: "codex", key, install: "/api/codex-setup/", win: "/api/codex-setup-win/", defaultModel: cat.defaultModel, catalog: cat.json, ccName: "CC Team" }]).replace(/</g, "\\u003c")};
</script>
<script src="${assets.url("ui.js")}"><\/script>
<script src="${assets.url("setup-guide.js")}"><\/script>
</body></html>`;
}

// codex(可选):{ noProfile, catalog } —— 页内「配置 Codex」分区所需的服务端数据。
// claude(可选):{ noProfile, hints } —— 页内「配置 Claude Code」分区所需的服务端数据。
// 两者缺省都按空处理,旧调用点不破。
// 评审详情弹层的外壳 —— 管理面板与「我的用量」共用同一份结构(样式在 theme.css,
// 行为在 ui.js)。详情不再堆在列表下面:弹层里带搜索 + 分页,只渲染当前页。
// extraHead 用来塞页面各自的东西(管理面板是「导出报告」)。
function reviewDetailDialog(id, extraHead = "") {
  return `<div class="dlg-overlay" id="${id}" role="dialog" aria-modal="true" aria-labelledby="${id}Title">
  <div class="dlg-card">
    <div class="dlg-head">
      <span class="dlg-title" id="${id}Title">评审详情</span>
      <span class="dlg-spacer"></span>
      ${extraHead}
      <button type="button" class="dlg-close" onclick="dlgClose('${id}')" aria-label="关闭">✕</button>
      <span class="dlg-sub" id="${id}Sub"></span>
    </div>
    <div class="dlg-tools">
      <input type="search" class="dlg-search" id="${id}Search" placeholder="搜索文件路径或问题内容…" aria-label="搜索评审意见">
      <span class="dlg-count" id="${id}Count"></span>
    </div>
    <div class="dlg-body" id="${id}Body"></div>
    <div class="dlg-foot">
      <span id="${id}Info"></span>
      <button type="button" class="pager-btn" id="${id}Prev">← 上一页</button>
      <button type="button" class="pager-btn" id="${id}Next">下一页 →</button>
    </div>
  </div>
</div>`;
}

export function personalUsageHtml(d, virtualKey, codex, claude, review) {
  const { assets } = d;
  const cxCat = (codex && codex.catalog) || { entries: [], json: '{\n  "models": []\n}', defaultModel: "" };
  const cxBanner = codex && codex.noProfile
    ? `<div style="background:#fff7e6;border:1px solid #ffe1a6;color:#a1662f;padding:10px 14px;border-radius:6px;margin-bottom:16px;font-size:13px">该 Key 尚未分配到任何 Responses(Codex) 方案——请联系管理员在设置页为其分配后再来配置。</div>`
    : "";
  const clBanner = claude && claude.noProfile
    ? `<div style="background:#fff7e6;border:1px solid #ffe1a6;color:#a1662f;padding:10px 14px;border-radius:6px;margin-bottom:16px;font-size:13px">该 Key 尚未分配到任何 Anthropic(Claude Code) 方案——请联系管理员在设置页为其分配后再来配置。</div>`
    : "";
  // 侧栏图标:统一容器,只有路径不同。stroke-width 与 18px 的显示尺寸是配套的 ——
  // 24 单位网格缩到 18px 是 0.75 倍,2.5 × 0.75 = 1.875px 的等效线宽(细线图标那种
  // 1.5px 观感是明确要避开的)。改 width/height 必须同改 stroke-width,
  // 并同步 my-usage.css 里 .nav-btn svg 上方的注释。
  const navIco = (paths) => `<svg class="nav-ico" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
  const ICO = {
    overview: navIco('<rect x="3.5" y="3.5" width="7" height="7" rx="2"/><rect x="13.5" y="3.5" width="7" height="7" rx="2"/><rect x="3.5" y="13.5" width="7" height="7" rx="2"/><rect x="13.5" y="13.5" width="7" height="7" rx="2"/>'),
    rates: navIco('<path d="M9.5 6.5h10"/><path d="M9.5 12h10"/><path d="M9.5 17.5h10"/><circle cx="4.2" cy="6.5" r="1.1" fill="currentColor" stroke="none"/><circle cx="4.2" cy="12" r="1.1" fill="currentColor" stroke="none"/><circle cx="4.2" cy="17.5" r="1.1" fill="currentColor" stroke="none"/>'),
    analysis: navIco('<path d="M4 4v16h16"/><path d="M7.5 15.5 11 11.5l3 2.5 5-6"/>'),
    // 会话。尾巴是**填实**的,不是两条描边收拢:stroke-width 2.5 下那两条线会把小三角
    // 糊成一个实心疙瘩(和下面奖杯那条注释是同一个坑),索性直接填,结果可预期。
    // 容器上是 fill="none" stroke="currentColor",所以子路径必须自带 fill/stroke,
    // 漏了不会报错,只是尾巴画不出来 —— my-usage.js 里皇冠那段注释记过同一件事。
    sessions: navIco('<path d="M7.5 4h9a4 4 0 0 1 4 4v3a4 4 0 0 1-4 4h-9a4 4 0 0 1-4-4v-3a4 4 0 0 1 4-4Z"/><path d="M8.5 15h5L11 20Z" fill="currentColor" stroke="none"/>'),
    // 奖杯。台阶/柱状在这里走不通:stroke-width 2.5 下三根独立竖线要么挨太近糊成一块,
    // 要么拉开到只剩细棍;而闭合的「天际线」路径会自带一条底边,实拍出来读作
    // 组织架构图/文件夹树,不像排行榜。奖杯没有这个歧义。
    leaderboard: navIco('<path d="M6.5 5.5h11v4.5a5.5 5.5 0 0 1-11 0Z"/><path d="M6.5 7.5a2.5 2.5 0 0 0 0 5"/><path d="M17.5 7.5a2.5 2.5 0 0 1 0 5"/><path d="M12 15.5v3"/><path d="M8 18.5h8"/>'),
    // 代码评审:一个带对勾的清单,读作「逐条看过」
    review: navIco('<rect x="4" y="3.5" width="16" height="17" rx="2.5"/><path d="M8 9h8"/><path d="M8 13h4"/><path d="M8 16.5h8"/>'),
    // 通知设置:铃铛
    notify: navIco('<path d="M12 3.5a5.5 5.5 0 0 0-5.5 5.5v3.2L5 15.5h14l-1.5-3.3V9A5.5 5.5 0 0 0 12 3.5Z"/><path d="M9.8 18a2.3 2.3 0 0 0 4.4 0"/>'),
    // 配置 Agent(Claude Code / Codex / MCP 三合一菜单)。「插头」:两根插脚 + 圆角插头体
    // + 接线,对应「把网关能力插进 Agent」。圆体用 path 弧线而非 rect+大 rx(描边下矩形
    // 圆体读作电池);插脚用两根短线。
    agent: navIco('<path d="M9 4.5v3"/><path d="M15 4.5v3"/><path d="M6.8 7.5h10.4v3a5.2 5.2 0 0 1-10.4 0Z"/><path d="M12 15.7v3.8"/>'),
    // 侧栏底部的「文档」链接:书页图标(与 dashboard 的文档同款),跳 wiki 的我的用量页
    docs: navIco('<path d="M5 4.5h9l5 5V19.5H5Z"/><path d="M14 4.5v5h5"/><path d="M8.5 13h7M8.5 16h7"/>'),
  };
  return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2096%2096%22%3E%3Crect%20width%3D%2296%22%20height%3D%2296%22%20rx%3D%2222%22%20fill%3D%22%232f6e50%22%2F%3E%3Cg%20fill%3D%22none%22%20stroke%3D%22%23fbfbf8%22%20stroke-width%3D%2213%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20transform%3D%22translate(48%2048)%20scale(0.88)%20translate(-48%20-48)%22%3E%3Cpath%20d%3D%22M37%2026.5H31.5Q20.5%2026.5%2020.5%2037.5V58.5Q20.5%2069.5%2031.5%2069.5H37%22%2F%3E%3Cpath%20d%3D%22M59%2026.5H64.5Q75.5%2026.5%2075.5%2037.5V58.5Q75.5%2069.5%2064.5%2069.5H59%22%2F%3E%3C%2Fg%3E%3Ccircle%20cx%3D%2248%22%20cy%3D%2248%22%20r%3D%226.2%22%20fill%3D%22%23fbfbf8%22%2F%3E%3C%2Fsvg%3E">
...
<div class="layout">
<aside class="sidebar">
<div class="sidebar-hd"><div class="sidebar-brand"><svg class="brand-logo" width="34" height="34" viewBox="0 0 96 96" aria-hidden="true"><rect width="96" height="96" rx="22" fill="#2f6e50"/><g fill="none" stroke="#fbfbf8" stroke-width="11" stroke-linecap="round" stroke-linejoin="round" transform="translate(48 48) scale(0.9) translate(-48 -48)"><path d="M37 26.5H31.5Q20.5 26.5 20.5 37.5V58.5Q20.5 69.5 31.5 69.5H37"/><path d="M59 26.5H64.5Q75.5 26.5 75.5 37.5V58.5Q75.5 69.5 64.5 69.5H59"/></g><circle cx="48" cy="48" r="4.95" fill="#fbfbf8"/></svg><div><h1>我的用量</h1></div></div></div>
<nav class="sidebar-nav" id="myUsageNav" role="tablist" aria-label="功能菜单"><button type="button" class="nav-btn active" role="tab" id="mu-tab-overview" aria-controls="mu-panel-overview" aria-selected="true" data-section="overview">${ICO.overview}概览</button><button type="button" class="nav-btn" role="tab" id="mu-tab-rates" aria-controls="mu-panel-rates" aria-selected="false" tabindex="-1" data-section="rates">${ICO.rates}配额价目表</button><button type="button" class="nav-btn" role="tab" id="mu-tab-analysis" aria-controls="mu-panel-analysis" aria-selected="false" tabindex="-1" data-section="analysis">${ICO.analysis}用量分析</button><button type="button" class="nav-btn" role="tab" id="mu-tab-sessions" aria-controls="mu-panel-sessions" aria-selected="false" tabindex="-1" data-section="sessions">${ICO.sessions}会话使用情况</button><button type="button" class="nav-btn" role="tab" id="mu-tab-leaderboard" aria-controls="mu-panel-leaderboard" aria-selected="false" tabindex="-1" data-section="leaderboard">${ICO.leaderboard}团队排行榜</button><button type="button" class="nav-btn" role="tab" id="mu-tab-review" aria-controls="mu-panel-review" aria-selected="false" tabindex="-1" data-section="review">${ICO.review}<span style="display:inline-flex;align-items:center;gap:6px">代码评审<span id="mrPendingBadge" style="display:none;background:var(--orange);color:#fff;border-radius:8px;font-size:10px;padding:1px 6px;vertical-align:1px"></span></span></button><button type="button" class="nav-btn" role="tab" id="mu-tab-notify" aria-controls="mu-panel-notify" aria-selected="false" tabindex="-1" data-section="notify">${ICO.notify}通知设置</button><button type="button" class="nav-btn" role="tab" id="mu-tab-agent" aria-controls="mu-panel-agent" aria-selected="false" tabindex="-1" data-section="agent">${ICO.agent}配置 Agent</button></nav>
<div class="side-ft"><a class="nav-btn" href="/wiki/#/my-usage" target="_blank" rel="noopener">${ICO.docs}文档</a></div>
</aside>
<main class="main">
<div class="toolbar"><div class="proto-seg" id="protoSeg" role="group" aria-label="协议分类"><button type="button" class="on" data-proto="">全部</button><button type="button" data-proto="anthropic">Anthropic</button><button type="button" data-proto="responses">OpenAI</button></div><select id="profileSel" onchange="switchProfile(this.value)"><option value="all">全部可用方案</option></select><button type="button" id="qrBtn" class="qr-open-btn" style="display:none" onclick="openQrModal()">申请加量</button><div class="notice-wrap" id="noticeWrap" style="margin-left:auto"><button type="button" class="notice-bell" id="noticeBell" aria-label="消息通知" aria-haspopup="dialog" title="消息通知"><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5a5.5 5.5 0 0 0-5.5 5.5v3.2L5 15.5h14l-1.5-3.3V9A5.5 5.5 0 0 0 12 3.5Z"/><path d="M9.8 18a2.3 2.3 0 0 0 4.4 0"/></svg><span class="notice-badge" id="noticeBadge" hidden></span></button><div class="notice-pop" id="noticePop" hidden><div class="notice-pop-hd">未读消息 <span id="noticePopCount"></span></div><div class="notice-pop-list" id="noticePopList"></div><div class="notice-pop-ft"><button type="button" class="notice-link" onclick="openNoticeModal()">查看全部</button></div></div></div></div>
<div class="pane active" id="mu-panel-overview" role="tabpanel" aria-labelledby="mu-tab-overview">
...
<div class="pane" id="mu-panel-rates" role="tabpanel" aria-labelledby="mu-tab-rates" hidden>
...
<div class="pane" id="mu-panel-analysis" role="tabpanel" aria-labelledby="mu-tab-analysis" hidden>
<div class="lb-ctl" id="anaRangeCtl"></div>
<div class="chart-row">
<div class="box"><h3>今日24小时趋势</h3><canvas id="hourChart"></canvas></div>
<div class="box"><h3>用量趋势 <span id="trendTitle" style="font-size:11px;color:var(--dim);font-weight:400"></span></h3><canvas id="trendChart"></canvas></div>
</div>
<div class="box"><h3>模型请求 <span id="modelTitle" style="font-size:11px;color:var(--dim);font-weight:400"></span></h3><table id="modelTable"><thead><tr><th>模型</th><th class="n">请求数</th><th class="n">实际 Token</th><th class="n">倍率</th><th class="n">计入配额</th></tr></thead><tbody></tbody></table><div class="note" id="modelTableNote" style="font-size:11px;color:var(--dim);margin-top:8px"></div></div>
<div class="box" id="clientBox" style="display:none"><h3>客户端用量 <span style="font-size:11px;color:var(--dim);font-weight:400">识别到的调用方 · 依据请求头判定，可被伪造，仅供参考</span></h3><table id="clientTable"><thead><tr><th>客户端</th><th class="n">请求数</th><th class="n">实际 Token</th><th class="n">占比</th></tr></thead><tbody></tbody></table><div class="note" id="clientTableNote" style="font-size:11px;color:var(--dim);margin-top:8px"></div></div>
<div class="box" id="projDist" style="display:none"><h3>项目分布 <span style="font-size:11px;color:var(--dim);font-weight:400">只显示你参与过的项目 · token 与缓存率按会话归属</span> <span id="projScope" style="font-size:11px;color:var(--dim);font-weight:400"></span></h3><table id="projDistTable"><thead><tr><th>项目</th><th class="n">文件</th><th class="n">净产出</th><th class="n">Token</th><th class="n">缓存率</th></tr></thead><tbody></tbody></table><div class="note" id="projDistNote" style="font-size:11px;color:var(--dim);margin-top:8px"></div></div>
</div>
<div class="pane" id="mu-panel-sessions" role="tabpanel" aria-labelledby="mu-tab-sessions" hidden>
<div class="box" id="sessBox"><h3>会话使用情况 <span style="font-size:11px;color:var(--dim);font-weight:400">会话是使用习惯的最小单位 · 长会话连续用更省缓存</span></h3><div class="lb-ctl" id="sessRangeCtl"></div><div class="cards" id="sessSummary" style="margin-bottom:8px"></div><div id="sessBoard"></div><div class="note" id="sessNote" style="font-size:11px;color:var(--dim);margin-top:8px"></div></div>
</div>
<div class="pane" id="mu-panel-leaderboard" role="tabpanel" aria-labelledby="mu-tab-leaderboard" hidden>
<div class="box"><h3>团队排行榜 <span class="lb-sub" id="lbSub"></span></h3><div class="lb-ctl"><div class="pill-seg" id="lbDim" role="tablist" aria-label="排序维度"></div><div class="pill-seg" id="lbWin" role="tablist" aria-label="时间窗"></div></div><div class="lb-note" id="lbNote"></div><div id="lbBoard"></div></div>
</div>
<div class="pane" id="mu-panel-agent" role="tabpanel" aria-labelledby="mu-tab-agent" hidden>
<div class="pill-seg" id="agentTabs" role="tablist" aria-label="配置 Agent 分类" style="margin-bottom:12px"><button type="button" class="on" data-agent-tab="claude">Claude Code</button><button type="button" data-agent-tab="codex">Codex</button><button type="button" data-agent-tab="mcp">MCP</button></div>
<div data-agent-body="claude">
${claudeSetupBodyHtml((claude && claude.hints) || null, clBanner)}
</div>
<div data-agent-body="codex" hidden>
${codexSetupBodyHtml(cxCat, cxBanner)}
</div>
<div data-agent-body="mcp" hidden>
${mcpSetupBodyHtml(virtualKey)}
</div>
</div>
<div class="pane" id="mu-panel-review" role="tabpanel" aria-labelledby="mu-tab-review" hidden>
<div class="box"><h3>代码评审 <span style="font-size:11px;color:var(--dim);font-weight:400">你是成员的仓库的评审结果 · 只看得到自己负责的仓库</span></h3>
<div id="mrNote" class="note" style="font-size:11px"></div>
<div id="mrRepos"></div>
<div class="dlg-tools" style="border:1px solid var(--border);border-radius:6px;margin:14px 0 10px">
  <input type="search" class="dlg-search" id="mrRunSearch" placeholder="搜索仓库名或结论…" aria-label="搜索评审记录">
  <span class="dlg-count" id="mrRunCount"></span>
</div>
<div id="mrRuns"></div>
</div>
</div>
${reviewDetailDialog("mrDetailDlg", `<button type="button" class="pager-btn" id="mrDetailResolve" style="display:none">标记已解决</button>`)}
<div class="pane" id="mu-panel-notify" role="tabpanel" aria-labelledby="mu-tab-notify" hidden>
<div class="box"><h3>通知设置 <span style="font-size:11px;color:var(--dim);font-weight:400">你负责的仓库评出问题时,按下面的渠道提醒你</span></h3>
<div class="mn-note" id="mnNote">渠道信息明文回显,方便核对;清空某一项保存即清除该渠道。</div>
<label class="mn-check"><input type="checkbox" id="mnEnabled"><span>启用我的评审通知</span></label>
<div class="mn-grid">
  <div class="mn-field"><label>飞书机器人 Webhook</label><input type="text" id="mnFeishu" placeholder="https://open.feishu.cn/open-apis/bot/v2/hook/..."></div>
  <div class="mn-field"><label>钉钉机器人 Webhook</label><input type="text" id="mnDingtalk" placeholder="https://oapi.dingtalk.com/robot/send?access_token=..."></div>
  <div class="mn-field"><label>企业微信机器人 Webhook</label><input type="text" id="mnWecom" placeholder="https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=..."></div>
  <div class="mn-field"><label>Server酱 SendKey</label><input type="text" id="mnServerchan" placeholder="SCT..."></div>
  <div class="mn-field"><label>Bark Device Key</label><input type="text" id="mnBarkKey" placeholder="iOS 装 Bark 后复制的 Key"></div>
  <div class="mn-field"><label>收件邮箱 <span class="lbl-hint">(需管理员配好 SMTP)</span></label><input type="text" id="mnEmail" placeholder="you@corp.com"></div>
</div>
<div class="mn-actions">
  <button type="button" class="mu-btn mu-btn-outline" onclick="testMyNotify()">发送测试</button>
  <button type="button" class="mu-btn mu-btn-primary" onclick="saveMyNotify()">保存</button>
</div>
<div class="mn-status" id="mnStatus" role="status"></div>
</div>
</div>
</main>
</div>
<div class="cal-tip" id="calTip"></div>
<div class="modal-overlay" id="noticeModal" onclick="if(event.target===this)closeNoticeModal()"><div class="qr-modal" role="dialog" aria-modal="true" aria-label="消息通知" style="max-width:620px"><div class="qr-mhd"><b>消息通知</b><button type="button" class="qr-close" onclick="closeNoticeModal()" aria-label="关闭">✕</button></div><div class="qr-mbody"><div class="notice-tools"><span class="notice-sub" id="noticeSub"></span><button type="button" class="mu-btn mu-btn-outline" id="noticeReadAll" onclick="markAllNoticesRead()" style="display:none">全部已读</button></div><div id="noticeList"></div></div></div></div>
...
${(review && review.needEmail) ? `<div class="modal-overlay open" id="emailGate" style="z-index:70"><div class="qr-modal" role="dialog" aria-modal="true" aria-labelledby="emailGateTitle" style="max-width:430px"><div class="qr-mhd"><b id="emailGateTitle">请填写你的邮箱</b></div><div class="qr-mbody"><div class="qr-info" style="margin-bottom:0">代码评审的结果会按 <b>git 提交邮箱</b> 只通知到本人 —— 系统还不知道你的邮箱。填一次即可，之后不再询问（随时可在「通知设置」里修改）。</div><div class="qr-form"><label>邮箱 <i>*</i></label><input type="email" id="gateEmail" placeholder="you@corp.com" autocomplete="email" maxlength="200"><div id="gateErr" role="alert" style="color:var(--red);font-size:11px;min-height:16px;margin:4px 0 0"></div></div><div class="qr-actions"><button type="button" class="btn-checkin" id="gateSubmit">保存</button></div></div></div></div>` : ""}
<div id="mascotDock"></div>
<script src="${assets.url("ui.js")}"><\/script>
<script src="${assets.url("md.js")}"><\/script>
<script>
const VK='${escJs(virtualKey)}';
// 评审邮箱门:服务端判定(codeReview 启用且 member_notify 还没填邮箱)时随页面注入,
// 页面里那个不可关闭的弹窗已由服务端直接渲染成打开状态,JS 只负责提交逻辑。
const REVIEW_EMAIL_GATE=${JSON.stringify(!!(review && review.needEmail))};
</script>
<script src="${assets.url("my-usage.js")}"><\/script>
<script src="${assets.url("mascot.js")}"><\/script>
<script>
// 两个接入指南分区的数据注入(setup-guide.js 依赖 SETUP_GUIDES 这个全局,必须**先注入后加载**)。
// 只放「不随 host 变化」的东西:地址相关的全部由客户端按 host 输入框现算。
// 用 var 而非 const —— 同一页被引用两次时 const 会抛重声明错。
// < 转义成 \\u003c:别名是管理员可控字符串,含 <\\/script> 时会截断脚本块。
// (斜杠必须转义:HTML 解析器不认 JS 注释,正文里出现未转义的结束标签会在那里把整块切掉。)
var SETUP_GUIDES=${JSON.stringify([
    { guide: "claude", key: virtualKey || "", install: "/api/claude-setup/", win: "/api/claude-setup-win/", basePath: (claude && claude.hints && claude.hints.basePath) || "", modelLine: (claude && claude.hints && (claude.hints.aliases || []).join(" / ")) || "", ccName: "CC Team" },
    { guide: "codex", key: virtualKey || "", install: "/api/codex-setup/", win: "/api/codex-setup-win/", defaultModel: cxCat.defaultModel, catalog: cxCat.json, ccName: "CC Team" },
    { guide: "mcp", key: virtualKey || "", ccName: "CC Team" },
  ]).replace(/</g, "\\u003c")};
</script>
<script src="${assets.url("setup-guide.js")}"><\/script>
</body></html>`;
