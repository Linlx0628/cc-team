    ...
    try {
      for (let ci = 0; ci < candidateList.length; ci++) {
        ...
        try {
          ...
          if (isStreamRequest) {
            await handleStreamingProxy(req, res, cbody, reqHeaders, apiKey, creqModel, timeout, reqSource, cruntime, csuffix, candStrippedUrl, clientState, sessionSignal);
          } else {
            await handleJsonProxy(req, res, cbody, reqHeaders, apiKey, creqModel, timeout, reqSource, cruntime, csuffix, candStrippedUrl, clientState, sessionSignal);
          }
          served = true;
          servedBy = cand.name;
          break;
...
async function handleJsonProxy(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session) {
    ...
    try {
      ...
      try {
        ...
        if (upRes.statusCode >= 400) {
          ...
          if (usage) {
            recordUsage(apiKey, usage, json.model, suffix, runtime, session);
...
async function streamUpstreamOnce(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session) {
  const runtime = _rt || d.rt;
  throwIfClientAborted(clientState);
  const opts = {
    ...
    const upReq = transport.request(opts, (upRes) => {
      ...
      upRes.on("end", () => {
        ...
        if (usageHasTokens(usage)) {
          recordUsage(apiKey, usage, model, suffix, runtime, session);
...
async function handleStreamingProxy(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session) {
  let healUsed = false;
  for (;;) {
    try {
      await streamUpstreamOnce(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session);
      return;
    } catch (err) {
