...
// 两处不会同屏出现,DOM id 保持一致,codex-setup.js 对两者通用。
...
// noProfile=true 时面板顶部提示未分配 Responses 方案;catalog 缺省按空处理。
export function personalUsageHtml(d, virtualKey, codex) {
  const { assets } = d;
...
<script>
// 配置 Codex 分区的数据注入(codex-setup.js 依赖这三个全局;KEY 与本页的 VK 是同一个虚拟 Key)
const KEY=${JSON.stringify(virtualKey||"")};
const MODELS=${JSON.stringify(cxCat.json)};
const DEFAULT_MODEL=${JSON.stringify(cxCat.defaultModel)};
</script>
<script src="${assets.url("codex-setup.js")}"><\/script>
</body></html>`;
}
