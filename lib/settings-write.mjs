...
function settingsAuditSnapshot() {
  return {
    ...
    // 方案组调度三项走独立 JSON 路由，不经过 applySettings，所以这几行现在基本不会被触发。
    // 留着是防审计盲区：将来有人把它们挪进设置页表单时，改动不会静默不记。
    scheduleGroups: JSON.stringify(config.scheduleGroups || {}),
    scheduleRules: JSON.stringify(config.scheduleRules || {}),
    scheduleOverride: JSON.stringify(config.scheduleOverride || {}),
  };
}
...
function settingsAuditDiff(snap, now) {
  ...
  if (snap.scheduleGroups !== now.scheduleGroups) parts.push("方案组调度（命名方案组）");
  if (snap.scheduleRules !== now.scheduleRules) parts.push("方案组调度（时间规则）");
  if (snap.scheduleOverride !== now.scheduleOverride) parts.push("方案组手动指定");
