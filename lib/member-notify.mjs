...
//
// 渠道信息(webhook 地址、SendKey、邮箱)**明文回显**给成员本人:填的就是他们自己的
// webhook 与邮箱,掩码让他们无从核对填没填对(管理员要求全量明文显示)。写入的空值
// 语义见 save():null = 清除该字段,undefined/空串 = 保留(兼容旧客户端)。
...
export function createMemberNotify(d) {
  ...
  // (旧版的掩码视图已移除:渠道信息明文回显给成员本人,见文件头说明)
  ...
  function save(userKey, patch = {}) {
    ...
    return get(key);   // 明文回显:保存后界面直接重渲染刚存的值
  }
  ...
  return { get, save, markSeen, lastSeen, findByEmail };
}
