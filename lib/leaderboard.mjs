...
const DIMENSIONS = [
  ...
  },
  {
    ...
    hint: "每净增一行代码消耗的输出 token。越低越省。",
  },
  {
    key: "context_health", label: "上下文健康度", unit: "token/次", direction: "asc",
    hint: "平均单轮新增上下文 = 未命中缓存的输入 token / 请求数。越低说明缓存复用越充分、会话越连续。",
  },
  {
...
//
// 只有「窗口内真的有编辑」的人参与归一化。没有编辑的人如果也算进去,会把全队的 min
// 拉到 0(失败率 0、改写率 0),凭空抬高所有人的分。这类人质量分给 null,榜单排末尾。
//
// 少于 2 个编辑者时直接全员 null:三个信号必然全部 max===min,norm 全给 0.5,
// 于是人人 50 分。那看起来像个测量结果,其实不是 —— 单人群里「50 分」没有任何含义,
// 却会被当成真分数读。宁可留空并写明原因,由 note 字段承担说明。
function computeQuality(prodRows) {
  const eligible = prodRows.filter((r) => (r.edit_count || 0) > 0);
  if (eligible.length < 2) {
    return {
      scores: new Map(),
      note: eligible.length === 0
        ? "本期无人产生代码变更"
        : "本期仅 1 人有代码变更，质量分需要至少 2 人参与才有区分度",
    };
  }
...
export function createLeaderboardReader(d) {
  ...
  function getLeaderboard({ dimension, window: win, meKey } = {}) {
    ...
    const ensure = (key) => {
      ...
      if (!a) {
        a = {
          ...
          netLines: 0, tokenPerLine: null, quality: null,
        };
        acc.set(key, a);
      }
      return a;
    ...
    rows.sort((x, y) => {
      // 无数据的一律排末尾 —— 且不参与方向反转,否则升序维度会把 null 顶到第一名
      const xn = x.value == null, yn = y.value == null;
      if (xn && yn) return 0;
      if (xn) return 1;
      if (yn) return -1;
    ...
    return {
      ...
      unit: dim.unit,
      hint: dim.hint,
      // 维度清单随响应下发,前端据此建切换器 —— 单一事实来源,加维度只改这边
      dimensions: DIMENSIONS.map((x) => ({ key: x.key, label: x.label, unit: x.unit, direction: x.direction })),
      ...
      // 只在当前维度确实退化时才有值(目前只有代码质量会走到),界面原样展示
      note: dimKey === "code_quality" ? qualityNote : null,
      rows: out,
