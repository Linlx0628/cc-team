...
// 组装报文。fromName 可选:给显示名时 From 为 `RFC2047 编码名 <裸地址>`,非 ASCII
// (中文显示名)走 headerValue 编码;不给则 From 只放裸地址(与旧版逐字节一致)。
// html 可选:给了就产出 multipart/alternative(text 降级段 + html 段),客户端自挑能
// 渲染的那段;不给时输出与纯文本版**逐字节一致**(既有测试钉死了这个形状)。
// boundary 用 "----tm-<hex>":"-" 不在 base64 字母表里,不可能与编码后的正文相撞。
export function buildMessage({ from, fromName, to, subject, text, html }) {
  const name = String(fromName == null ? "" : fromName).trim();
  const base = [
    `From: ${name ? `${headerValue(name)} <${from}>` : from}`,
    `To: ${to.join(", ")}`,
  ...
  const htmlBody = String(html == null ? "" : html).trim();
  if (!htmlBody) {
    return [...base,
  ...
  const b = `----tm-${crypto.randomUUID().replace(/-/g, "")}`;
  return [...base,
...
// 发一封信。cfg: { host, port, secure, user, pass, from, to(单个或数组), subject, text, html, insecure }
//   insecure=true 跳过证书校验(自签名证书的内网 SMTP 很常见)——**默认关**,由管理员显式打开。
//   html 为可选的 HTML 正文(评审报告),只进 base64 编码的正文段,不碰命令/头部,无注入面。
export function sendMail(cfg, { timeoutMs = 12000, clientName = "token-monitor" } = {}) {
  return new Promise((resolve, reject) => {
    let host, port, from, fromName, tos, subject, text, html, user, pass, secure, insecure;
    try {
      ...
      html = String(cfg.html == null ? "" : cfg.html);
    ...
    (async () => {
      ...
      await session.payload(buildMessage({ from, fromName, to: tos, subject, text, html }) + CRLF + ".");
