...
- Responses/Codex 缓存命中不计入配额（向 Anthropic 口径对齐，每方案可用 `cacheReadQuotaRate` 调回按比例/全价），Codex 这类整段重发上下文的 agent 不再被缓存读打穿额度
...
- 产出质量工作区：从工具调用流量提取净产出/失败率/重写率/验证密度/token 效率，项目自动识别（会话主导子树），支持一键导出周期报告（自包含 HTML）
- 等值成本：token 按可编辑的参考牌价折算 USD，按成员/方案统计，峰谷分时（可选高峰价+全局时段）；空转与错误循环实时告警（复用通知渠道）
- 成员产出画像：个人页查看自己的产出趋势、失败率与缓存命中率建议
- 隐私：仅统计结构化指标（行数/路径/命令分类），不存储任何代码内容；可在设置页关闭文件路径记录或整体停用
...
- 时间口径：所有用户可见的时钟时刻统一显示**北京时间(UTC+8)**；存储层时间戳为 UTC/ISO，仅展示出口做 +8 转换
...
- Claude Code/ZCode（含 tool_result 内截图）与 Codex 均生效；同图按内容哈希缓存并持久化到 SQLite（网关重启不失效、不重转）；单图 12MB；单轮新识别图片超过 8 张时自动识别最近 8 张、更早的以占位说明替换（长会话重启后永不硬失败）；辅助池为空时返回明确的 400 提示
...
每个方案可以配置配额倍率，把「消耗多少 token」和「扣多少额度」解耦。配额口径按**协议**区分：Anthropic 方案的 `input_tokens` 本就不含缓存读；Responses/Codex 方案的上游把缓存命中折在 `input_tokens` 里，网关按 Anthropic 口径把它剔除后才计权（缓存命中默认不计入配额，可用方案的 `cacheReadQuotaRate` 调回：0=不计 / 0.x=按比例 / 1=按旧行为全价）。

```text
扣除额度 = (计权输入 + 输出) × 该模型在当前时段的倍率
计权输入 = 未命中的输入（responses/Codex 已剔除缓存命中；anthropic 本就如此）
```
...
- **缓存命中剔除（Responses/Codex）只作用于配额判定**：`input_tokens`/`cache_read` 等统计列仍按上游原样记录真实 token（成员用量页的「今日用量/缓存命中」卡片、生产成本报表都不变），只是从 `weighted_tokens` 的计费输入里减掉命中的部分
- 只有上游回报缓存子集（`cached_tokens`/`cache_read_input_tokens`）时才剔除；不回报该字段的上游照旧全价计入（信息缺失，非 bug）
- 统计报表、图表、导出始终显示真实 token，倍率与缓存剔除都只作用于配额判定
