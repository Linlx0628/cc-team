    ...
    try {
      for (let ci = 0; ci < candidateList.length; ci++) {
        ...
        try {
          ...
          if (cruntime.protocol === "responses" && translateCompactionInputItems(parsed)) dirty = true;
...
async function handleStreamingProxy(req, res, body, reqHeaders, apiKey, reqModel, timeout, reqSource, _rt, suffix, strippedUrl, clientState, session, client) {
  ...
  for (;;) {
    try {
      ...
      // 两种自愈(tool-pattern / thinking 回传)共用一次重发机会,与 handleJsonProxy 同语义
      if ((!err?.toolPatternRetry && !err?.thinkingRetry) || healUsed) throw err;
      healUsed = true;
      body = err.body;
    }
  }
