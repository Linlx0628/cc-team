...
<div class="section">
  ...
  <div id="crRepoEditor" hidden style="margin-top:12px;padding-top:12px;border-top:1px dashed var(--border)">
    ...
    <div class="row" style="margin-top:10px">
      ...
      <div><label>分支 <span class="lbl-hint">(可点「拉取分支」从仓库列出来选)</span></label>
        <div style="display:flex;gap:6px">
          <input type="text" id="crRepoBranch" list="crRepoBranchList" placeholder="main" style="flex:1">
          <button type="button" class="btn btn-outline btn-sm" onclick="fetchReviewRepoBranches()">拉取分支</button>
        </div>
        <datalist id="crRepoBranchList"></datalist>
      </div>
    </div>
    <div class="row" style="margin-top:10px">
      ...
      <div><label>凭据 <span class="lbl-hint">(留空 = 不修改)</span></label><input type="text" id="crRepoCred" placeholder="Token 或私钥绝对路径"></div>
    </div>
...
<section class="chart-workspace" aria-label="用量图表">
  ...
  <div class="chart-panel chart-profile"><div class="chart-head"><h2>方案请求情况</h2><span class="chart-note" id="profileNote"></span></div><div class="chart-canvas"><canvas id="profileChart"></canvas></div></div>
    ...
    ${reviewDetailDialog("crDetailDlg", `<a class="pager-btn" id="crDetailExport" href="#" style="display:none">导出报告</a><button type="button" class="pager-btn" id="crDetailResolve" style="display:none">标记已解决</button>`)}
...
export function personalUsageHtml(d, virtualKey, codex, claude, review) {
  ...
  const ICO = {
    ...
    codex: navIco('<path d="M3.5 7.5h3"/><path d="M13.5 7.5h7"/><path d="M3.5 16.5h7"/><path d="M17.5 16.5h3"/><circle cx="10" cy="7.5" r="2.3"/><circle cx="14" cy="16.5" r="2.3"/>'),
    // 配置 Claude Code。「终端窗口」而不是 Anthropic 那三笔画成的 A：A 在 18px / stroke-width 2.5
    // 下三笔会粘成一团（与 sessions 的尾巴、leaderboard 的奖杯是同一类坑），而终端窗口有外框
    // 兜住，缩小后仍读得出是「命令行工具」，正好对上 Claude Code 是 CLI 这件事。
    // 改 width/height 必须同改 stroke-width（见 navIco 上方注释）。
    claude: navIco('<rect x="3.2" y="4.2" width="17.6" height="15.6" rx="2.6"/><path d="M7.6 9.8 9.9 12l-2.3 2.2"/><path d="M12.6 14.4h4"/>'),
    // 配置 MCP。「插头」:两根插脚 + 圆角插头体 + 接线,对应「把网关能力插进 Claude」。
    // 圆体用 path 弧线而非 rect+大 rx(描边下矩形圆体读作电池);插脚用两根短线。
    mcp: navIco('<path d="M9 4.5v3"/><path d="M15 4.5v3"/><path d="M6.8 7.5h10.4v3a5.2 5.2 0 0 1-10.4 0Z"/><path d="M12 15.7v3.8"/>'),
  };
  return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2096%2096%22%3E%3Crect%20width%3D%2296%22%20height%3D%2296%22%20rx%3D%2222%22%20fill%3D%22%232f6e50%22%2F%3E%3Cg%20fill%3D%22none%22%20stroke%3D%22%23fbfbf8%22%20stroke-width%3D%2213%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20transform%3D%22translate(48%2048)%20scale(0.88)%20translate(-48%20-48)%22%3E%3Cpath%20d%3D%22M37%2026.5H31.5Q20.5%2026.5%2020.5%2037.5V58.5Q20.5%2069.5%2031.5%2069.5H37%22%2F%3E%3Cpath%20d%3D%22M59%2026.5H64.5Q75.5%2026.5%2075.5%2037.5V58.5Q75.5%2069.5%2064.5%2069.5H59%22%2F%3E%3C%2Fg%3E%3Ccircle%20cx%3D%2248%22%20cy%3D%2248%22%20r%3D%226.2%22%20fill%3D%22%23fbfbf8%22%2F%3E%3C%2Fsvg%3E">
...
<div class="pane" id="mu-panel-claude" role="tabpanel" aria-labelledby="mu-tab-claude" hidden>
${claudeSetupBodyHtml((claude && claude.hints) || null, clBanner)}
</div>
<div class="pane" id="mu-panel-codex" role="tabpanel" aria-labelledby="mu-tab-codex" hidden>
...
<div class="mn-note" id="mnNote">渠道凭据只存在服务器上,这里只显示是否已配置;输入框留空表示不修改。</div>
...
<div class="pane" id="mu-panel-mcp" role="tabpanel" aria-labelledby="mu-tab-mcp" hidden>
