...
export function createCodeReview(d) {
  ...
  async function ensureRepo(repo, dir) {
    const auth = { token: repo.authType === "token" ? repo.credential : null, username: repo.authType === "token" ? repo.username : null, sshKeyPath: repo.authType === "sshKey" ? repo.credential : null };
  ...
  function status() {
    ...
    return {
      ...
      // 面板下拉要的是仓库清单(id+名称);repos 保持数量不变(状态行「仓库 N 个」在用)
      repoList: (c.repos || []).map((r) => ({ id: r.id, name: r.name, enabled: r.enabled !== false })),
