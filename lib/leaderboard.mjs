...
//
// 缓存率直接复用 production.mjs 的 contextHealth。usage 存储口径已对齐(2026-09-18):
// 所有协议的 input_tokens 都不含缓存读,公式即标准口径,无需任何协议修正。
//
...
const DIMENSIONS = [
  ...
  },
  {
    ...
    hint: "输入(不含缓存读,各协议同口径) + 输出。这是消耗量,不等于效率 —— 与「效率比」对照看。",
  },
  {
...
export function createLeaderboardReader(d) {
  const { cnDate, cnWeekStartDate, productionSummary, contextHealth, knownUserKeys } = d;
  ...
  function getLeaderboard({ dimension, window: win, meKey } = {}) {
    ...
    const health = contextHealth(db, { from, to });
    ...
    const ensure = (key) => {
      ...
      if (!a) {
        a = {
          ...
          newInput: 0,          // 新增输入(对齐口径:不含缓存读),来自 contextHealth 的 denomI
