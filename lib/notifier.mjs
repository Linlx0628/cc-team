...
import { sendMail, isValidEmail } from "./smtp.mjs";
...
// send(n, msg, opts):前两个参数是所有渠道通用的;opts 目前只有邮件用得上
//   opts.to      收件人(覆盖「发给谁」—— IM 渠道固定发到自己的群/设备,邮件必须显式给)
//   opts.subject 邮件主题(IM 渠道没有主题概念,取正文首行即可)
// 其余渠道忽略 opts,因此给 send 加第三参不会影响它们。
const NOTIFY_SENDERS = [
  ...
  // 邮件:SMTP 直发。与上面几个不同,它**必须**知道收件人 —— 没给 opts.to 视为未启用,
  // 免得「配了 SMTP 却没指定收件人」时把信发给 From 自己。
  // opts.html 可选:HTML 正文(评审报告),smtp.mjs 会组 multipart/alternative。
  { channel: "邮件", enabled: (n) => !!String(n.smtpHost || "").trim() && !!String(n.smtpFrom || "").trim(), send: (n, msg, opts) => {
    const to = (opts && opts.to) || n.smtpTo || "";
    if (!to) return Promise.reject(new Error("未指定收件人"));
    return sendMail({
      host: n.smtpHost, port: n.smtpPort, secure: n.smtpSecure !== false, insecure: n.smtpInsecure === true,
      user: n.smtpUser, pass: n.smtpPass, from: n.smtpFrom, fromName: n.smtpFromName || "",
      to, subject: (opts && opts.subject) || msg.split("\n")[0], text: msg, html: (opts && opts.html) || "",
    }, { timeoutMs: NOTIFY_TIMEOUT_MS * 2 });   // 握手+TLS+投递比一条 webhook 慢,给两倍
  } },
];

// 成员**自助**渠道(存在 member_notify 表里,字段名与管理员那套一致,故复用同样的请求形状)。
// 邮件不在这里 —— 它要管理员配的 SMTP 凭据,单独处理(见 server.mjs 的 pushReviewToMembers)。
const MEMBER_SENDERS = [
  { channel: "飞书", enabled: (m) => !!String(m.feishuWebhook || "").trim(), send: (m, msg) =>
    postHttpRequest(m.feishuWebhook, { body: JSON.stringify({ msg_type: "text", content: { text: msg } }), contentType: "application/json" }) },
  { channel: "钉钉", enabled: (m) => !!String(m.dingtalkWebhook || "").trim(), send: (m, msg) =>
    postHttpRequest(m.dingtalkWebhook, { body: JSON.stringify({ msgtype: "text", text: { content: msg } }), contentType: "application/json" }) },
  { channel: "企业微信", enabled: (m) => !!String(m.wecomWebhook || "").trim(), send: (m, msg) =>
    postHttpRequest(m.wecomWebhook, { body: JSON.stringify({ msgtype: "text", text: { content: msg } }), contentType: "application/json" }) },
  { channel: "Server酱", enabled: (m) => !!String(m.serverchanSendKey || "").trim(), send: (m, msg) => {
    const key = String(m.serverchanSendKey).trim();
    const form = `title=${encodeURIComponent(msg.split("\n")[0])}&desp=${encodeURIComponent(msg)}`;
    return postHttpRequest(`https://sctapi.ftqq.com/${encodeURIComponent(key)}.send`, { body: form, contentType: "application/x-www-form-urlencoded" });
  } },
  { channel: "Bark", enabled: (m) => !!String(m.barkDeviceKey || "").trim(), send: (m, msg) => {
    const base = String(m.barkServer || "").trim().replace(/\/+$/, "") || "https://api.day.app";
    const key = encodeURIComponent(String(m.barkDeviceKey).trim());
    return postHttpRequest(`${base}/${key}`, { body: JSON.stringify({ body: msg, group: "token-monitor" }), contentType: "application/json" });
  } },
];
...
async function sendNotifierTest(cfg) {
  ...
  const results = await Promise.all(channels.map(async (s) => {
    // 邮件必须显式给收件人:优先用单独填的收件人,没填就发给发件人自己(自测最常见)
    const opts = s.channel === "邮件" ? { to: cfg.smtpTo || cfg.smtpFrom, subject: "[token-monitor] 通知测试" } : undefined;
    try { await s.send(cfg, msg, opts); return { channel: s.channel, ok: true }; }
...
function sanitizeNotifierConfig(input) {
  ...
  const parsedPort = parseInt(src.smtpPort, 10);
  return {
    enabled: !!src.enabled,
    ...
    // 邮件(SMTP 直发)。主机名/账号不允许空白与换行(SMTP 命令行注入的入口),
    // 端口钳到合法区间;465 = 隐式 TLS(secure 默认 true),587 走 STARTTLS。
    smtpHost: String(src.smtpHost || "").trim().replace(/[\s\r\n]+/g, "").slice(0, 253),
    smtpPort: Number.isFinite(parsedPort) ? Math.min(65535, Math.max(1, parsedPort)) : 465,
    smtpSecure: src.smtpSecure !== false,
    smtpUser: String(src.smtpUser || "").trim().slice(0, 200),
    smtpPass: String(src.smtpPass || "").slice(0, 400),
    smtpFrom: String(src.smtpFrom || "").trim().slice(0, 200),
    // 发件人显示名(可选):收件人看到的「发件人」名字,如「CC-TEAM-代码评审」。
    // 进头部前还有 headerValue 兜底剥换行(RFC2047 编码非 ASCII)。
    smtpFromName: String(src.smtpFromName || "").trim().slice(0, 100),
    smtpTo: String(src.smtpTo || "").trim().slice(0, 200),   // 管理员侧默认收件人(可留空,由各功能自行指定)
    // 内网自签名证书很常见。默认**必须**校验证书,由管理员显式勾选才跳过 ——
    // 绝不能因为「配置麻烦」而默认放行,那等于把邮件凭据暴露给中间人。
    smtpInsecure: src.smtpInsecure === true,
  };
}
